...
- a simplified Setup **Excel Planning Import** page: choose one `.xlsx` worksheet, preview the predefined Cases/Orders/Batches mapping, then review **Cases → related Orders → Batches in Pool → Machine assignment** before one Edit-Mode-gated atomic commit. The old outcome/column/Machine/pattern tab wizard is hidden from the normal workflow. The stages are focused views of the same preview, not separate imports. The importer never invents a Case route, Machine, allocation, or compatibility override, and the Server revalidates every proposed row;
