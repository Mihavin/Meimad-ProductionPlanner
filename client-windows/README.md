...
- a simple bounded local `.stp` / `.step` wireframe viewer in the Case workspace that fits only referenced model geometry on open, excludes coordinate-system/origin points from Fit, orbits around the centroid of the model vertices rather than the STEP origin, supports drag rotation, wheel zoom, standard views, and vertex-to-vertex distance/axis-delta measurement in model coordinate units; PNG snapshot capture uses the existing Case picture-path field and changes no Server data until the editor explicitly saves the Case;
...
- a guided Setup **Excel Import Wizard** that previews one legacy `.xlsx`, maps unclear source columns, then leads the operator through **Cases → related Orders → Batches in Pool → Machine assignment** before one Edit-Mode-gated atomic commit. The stages are focused views of the same preview, not separate imports. A selected new Case may become the source Case for related Order rows in that one transaction; Pool creates an existing routed Case's full route as unassigned Batch Operations. The wizard never invents a Case route, Machine, allocation, or compatibility override; optional Batch-number templates use only `{part}`, `{reference}`, and `{row}`, and the Server revalidates every expanded row;
