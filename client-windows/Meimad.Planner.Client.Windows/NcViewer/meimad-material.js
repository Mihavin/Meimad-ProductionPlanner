/*
 * Meimad Planner material removal core (pure functions, no DOM): a 2.5D stock for milling
 * (one top height per grid point, lowered by the swept tool along every motion) and an outer /
 * inner radius profile per Z column for turning (cut by the nose circle and the insert's body
 * side), plus STL reading and writing. Loaded by the page, by the simulation worker
 * (importScripts) and by the V8 test host.
 */
(function (root, factory) {
  ...
  function lower(stock, index, ix, edge) {
    ...
    if (limited <= stock.bottom + EPSILON && current > stock.bottom + EPSILON) stock.throughCut += 1;
  ...
  // Cuts one motion: points [{x, y, z}, ...] of the tool tip.
  function cutMillSegment(stock, points, tool, axis) {
    if (!points || points.length === 0) return false;
    if (axis && axis.z < 0.985) return false; // tilted tool axis: not representable in 2.5D
    if (points.length === 1) return cutMillPiece(stock, points[0], points[0], tool);
    let changed = false;
    for (let index = 1; index < points.length; index += 1) {
      if (cutMillPiece(stock, points[index - 1], points[index], tool)) changed = true;
    }
    return changed;
  }
  ...
  // Mill: the top surface grid, walls down to the bottom where material meets air (the box
  // sides, or the boundary of a round blank) and the bottom.
  function millTriangles(stock) {
    const { nx, ny, cell, originX, originY, top, bottom } = stock;
    const triangles = [];
    ...
    for (let iy = 0; iy < ny - 1; iy += 1) {
      for (let ix = 0; ix < nx - 1; ix += 1) {
        ...
        push(a, c, d);
        if (stock.masked) {
          push([a[0], a[1], bottom], [c[0], c[1], bottom], [b[0], b[1], bottom]);
          push([a[0], a[1], bottom], [d[0], d[1], bottom], [c[0], c[1], bottom]);
        }
      }
    }
    // Vertical wall between two points along one grid edge, from the bottom up to their tops.
    const wall = (a, b) => {
      const a0 = [a[0], a[1], bottom];
      const b0 = [b[0], b[1], bottom];
      push(a, b0, b);
      push(a, a0, b0);
    };
    if (!stock.masked) {
      for (let ix = 0; ix < nx - 1; ix += 1) {
        wall(at(ix + 1, 0), at(ix, 0));
        wall(at(ix, ny - 1), at(ix + 1, ny - 1));
      }
      for (let iy = 0; iy < ny - 1; iy += 1) {
        wall(at(0, iy), at(0, iy + 1));
        wall(at(nx - 1, iy + 1), at(nx - 1, iy));
      }
    ...
    for (let iy = 0; iy < ny; iy += 1) {
      for (let ix = 0; ix < nx; ix += 1) {
        ...
        const p = at(ix, iy);
        if (!solid(ix + 1, iy)) wall([p[0] + half, p[1] - half, p[2]], [p[0] + half, p[1] + half, p[2]]);
        if (!solid(ix - 1, iy)) wall([p[0] - half, p[1] + half, p[2]], [p[0] - half, p[1] - half, p[2]]);
        if (!solid(ix, iy + 1)) wall([p[0] + half, p[1] + half, p[2]], [p[0] - half, p[1] + half, p[2]]);
        if (!solid(ix, iy - 1)) wall([p[0] - half, p[1] - half, p[2]], [p[0] + half, p[1] - half, p[2]]);
      }
    }
    return new Float32Array(triangles);
