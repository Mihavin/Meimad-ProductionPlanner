...
11. The database stores only a Case Working Folder path. Never modify original engineering files. Generated previews or cache data may be written only below that folder's `_MeimadPlanner` directory. NC programs that a planner saves or releases from the NC viewer for a Case Operation go below `Gcode\<Case name>\<Operation number>\<process revision>\<postprocessor name>\<local revision>\` in that folder; missing folders are created, and a different existing file is replaced only after the planner confirms it. The viewer's stock definition is saved beside the program as `<program>.stock.json`, and a machined stock exported for the next Operation goes to `Gcode\<Case name>\<next Operation number>\Stock\`.
...
15. Official E-Ink planning/package data flows server-to-device only. Tablets may download only the resources assigned by the Server and may send only the narrowly scoped `SEND_TO_QC` operational event for the Server-resolved active Production Run. Tablet communication does not require a device authentication credential in the MVP. `TabletID` is used only as the device identifier. `SEND_TO_QC` changes only the tablet workflow projection to `IN_QC`; it may not edit planning assignments, backlog order, quantities, allocations, execution counts, or package data. Official package files remain read-only in the device UI.
...
18. Restrict each tablet to the resources resolved by the Server for its `TabletID` plus its explicit `SEND_TO_QC` event scope. The Server, never the tablet, resolves the target Machine and active Production Run. The MVP intentionally does not use per-tablet tokens, API keys, passwords, shared secrets, certificates, or revocable device credentials. The factory LAN/Wi-Fi is treated as the trusted network boundary. This decision applies only to tablet communication and does not remove or weaken NC-file verification, CNC challenge/response, or Offset Loader authorization. Limit cached confidential data and keep device health and tablet workflow events separate from planning data.
...
28. A persistent CNC Setup/Production macro variable is not a workflow or verification authority and must not be reintroduced. Server workflow is projected from immutable Production Run operational events. Ordinary mutable CNC variables may cache or carry temporary handshake values, but clearing, resetting, wrapping, or editing them never establishes or invalidates Server authority by itself.
29. Every newly approved CNC NC release is an immutable canonical source template using the exact `[[MEIMAD:<KEY>]]` grammar defined in `docs/postprocessor-production-package-contract.md`. It contains server-owned identity/context placeholders, including `PART_NAME` and `OPERATION_NAME`, and exactly one standalone `VERIFICATION_HOOK` before executable code, and exactly one `CYCLE_START`/`CYCLE_END` pair around the physical part cycle (a template with neither marker fails package creation with `production_package_cycle_marker_required`). The postprocessor is server-blind and never supplies authoritative Planner identity. The Server validates the template, assigns the globally unique six-digit NC identity to the immutable release, and resolves a copy only while atomically building a Machine-specific Production Package. Verification-enabled packages receive the configured hook; verification-disabled packages contain no active hook or verification Offset Loader. Preserve the source release hash and separately record every generated artifact hash. The former `(MEIMAD PACKAGE ... V1)` grammar is an explicit read/build compatibility protocol for immutable historical releases only, not current postprocessor guidance.
30. CNC Machine identity consists only of the Planner `MachineID`, the configured fixed CNC IP address, and the configured controller MAC address. Do not add a Machine Secret, derived Machine key, HMAC key, password, token, certificate, or other CNC identity credential. IP and MAC changes are explicit configuration changes and do not silently establish a new identity.
31. CNC event sequence numbers are evidence only. Retain and report duplicate, gap, reset, wrap, and out-of-order observations, but never use sequence continuity alone to invalidate Machine identity, invalidate a valid NC/Offset Loader verification binding, force recovery, or block workflow. Event identity and exact Run/Machine/NC/Offset Loader evidence remain authoritative.
32. Setup verification follows `OFFSET_LOADER_COMPLETED -> ARMED -> PENDING -> SUCCEEDED`. `ARMED` has no timeout and may expose the response on the assigned tablet. Only the first intended NC start for the exact binding changes `ARMED` to `PENDING` and starts the configured timeout. A successful exact binding is reused on subsequent starts without another prompt. A newly completed Offset Loader supersedes any earlier armed, pending, or successful binding.
33. Machine identity, NC-file verification, and Offset Loader authorization are separate concerns. Recognizing a configured Machine never proves the NC or Offset Loader valid. The Server's immutable Production Run events, approved NC release and identity, and current Offset Loader release are the verification authority.
34. Preserve four resource classes: specialized Machine, Employee with data-managed Skills, generic internal Workstation with data-managed type/capabilities/capacity, and External Resource with lead-time facts but no invented supplier capacity. Operation/process definitions express requirements; provisional planning selects concrete resources. Machine assignment/backlog authority remains manual, while auxiliary resource assignment is an automatic deterministic prediction before manual override. Ordinary contention shifts the prediction; only missing/invalid configuration blocks it. Evaluate delivery risk only after a feasible resource-constrained timeline exists.
35. `Manual / Dummy Tool Offsets` is an explicit Machine capability and immutable Production Package input mode, not a Tool Room bypass. It omits measured offset payload and makes the setupist responsible for entering real offsets. A verification-enabled CNC package must still contain the package-specific anonymous-free Offset Loader identity and every normal verification/setup hook. Do not branch domain behavior on Machine numbers; schema/configuration enables the initial Machines 10, 14, and 15 pilot.
