...
29. Every newly approved CNC NC release is an immutable source template and must contain exactly one stable machine-readable package placeholder `(MEIMAD PACKAGE VERIFY V1 NCID=xxxxxx)` before its first executable block. The six-digit NC identity is globally unique and bound immutably to that exact release. Optional cycle placeholders must be absent or appear as exactly one START/END pair. The Server validates placeholder structure at release and resolves it only while atomically building a Machine-specific Production Package: verification-enabled CNC packages receive the configured hook and commissioned CST/CEN content; verification-disabled CNC packages have all Meimad package placeholders removed and receive no active Server-verification code. Preserve the source release hash and separately record every generated runnable artifact hash. Historical active-hook releases remain audit history but are not newly release-eligible without explicit re-release as a valid template.
