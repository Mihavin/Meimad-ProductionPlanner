...
/// <summary>
/// The controller's DPRNT output as one adapter-independent source: either the read-only
/// TCP stream (Haas port or a serial-to-Ethernet bridge) or a controller-written print file
/// tailed over a network share. Line semantics are identical for both. The FILE source also
/// owns the configured clear policy, because no G-code ever deletes that file.
/// </summary>
internal sealed class CncDprntSource : IAsyncDisposable
{
    ...
    internal string Description => UsesFile
        ? $"DPRNT file '{FilePath}'"
        : UsesTcp ? $"DPRNT TCP port {TcpPort} on {host}" : "DPRNT (not configured)";
    ...
    internal string SuccessMessage(CncDprntObservation observation) => UsesFile
