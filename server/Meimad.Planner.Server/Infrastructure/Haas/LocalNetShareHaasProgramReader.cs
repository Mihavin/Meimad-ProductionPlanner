...
internal sealed class LocalNetShareHaasProgramReader(TimeProvider timeProvider) : IHaasProgramReader
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var normalizedProgram = NormalizeProgram(programNumber);
        ...
        foreach (var path in paths.Take(10000))
        {
            ...
            if (located?.Success == true && $"O{located.Groups["number"].Value}" == normalizedProgram)
        ...
        {
            1 => new MachineNcHeader(normalizedProgram, candidates[0].Lines,
                candidates[0].Path, timeProvider.GetUtcNow()),
            0 => throw new HaasProgramHeaderUnavailableException(
                $"No readable machine-side NC file uniquely mapped to active program {normalizedProgram}."),
            _ => throw new HaasProgramHeaderUnavailableException(
                $"More than one machine-side NC file contains active program {normalizedProgram}; header identity is ambiguous.")
        };
    }
    ...
    private static string NormalizeProgram(string value)
    {
        ...
        if (!Regex.IsMatch(normalized, @"^O\d{1,8}$", RegexOptions.CultureInvariant))
            throw new HaasProgramHeaderUnavailableException("The active Haas program number was invalid.");
        return normalized;
    }
}
