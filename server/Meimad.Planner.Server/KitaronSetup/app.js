...
function sourceInput(field) {
  ...
  if (field.transform === "generated_working_folder" || field.connectorManaged) input.disabled = true;
...
function renderMappings() {
  ...
  mappingFields.forEach((field, index) => {
    ...
    enabled.disabled = field.connectorManaged || (field.required && applies);
    ...
    if (field.connectorManaged) {
      const managed = document.createElement("span");
      managed.className = "managed-mark";
      managed.textContent = " CONNECTOR MANAGED";
      title.append(managed);
    }
    ...
    confidence.disabled = field.connectorManaged;
    ...
    transform.disabled = field.connectorManaged;
...
function updateMappingSummary() {
  ...
  const active = current
    .map((field, index) => ({ ...field, connectorManaged: mappingFields[index].connectorManaged }))
    .filter((_, index) => mappingFields[index].modelModes.includes(mode));
  const enabled = active.filter(field => field.enabled);
  const blocked = enabled.filter(field => field.confidence === "blocked").length;
  const missing = enabled.filter(field => !field.connectorManaged && !field.sourceColumn
  ...
  const unknown = detected.size === 0 ? 0 : enabled.filter(field =>
    !field.connectorManaged && field.sourceColumn
