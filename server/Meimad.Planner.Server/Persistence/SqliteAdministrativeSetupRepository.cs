...
internal sealed class SqliteAdministrativeSetupRepository : IAdministrativeSetupRepository
{
    ...
    public async Task<IReadOnlyList<EmployeeResource>> ListResourcesAsync(CancellationToken token)
    {
        ...
        command.CommandText = "SELECT id, employee_number, name, resource_type, first_name, last_name, skills_json, assigned_calendar_id, photo_path, notes, email, is_active, version, created_at, updated_at FROM employee_resources ORDER BY employee_number COLLATE NOCASE, id;";
    ...
    public Task<EmployeeResource?> UpdateResourceAsync(EmployeeResource value, int expectedVersion, EditAuthority authority, CancellationToken token) =>
        WriteAsync<EmployeeResource?>(authority, async (connection, transaction) =>
        {
            ...
            await using var command = connection.CreateCommand(); command.Transaction = transaction;
            command.CommandText = "UPDATE employee_resources SET employee_number=$number,name=$name,resource_type=$type,first_name=$firstName,last_name=$lastName,skills_json=$skills,assigned_calendar_id=$calendar,photo_path=$photoPath,notes=$notes,email=$email,is_active=$active,version=$version,updated_at=$updated WHERE id=$id AND version=$expected;";
    ...
    { await using var command=c.CreateCommand(); command.Transaction=t; command.CommandText="SELECT id,employee_number,name,resource_type,first_name,last_name,skills_json,assigned_calendar_id,photo_path,notes,email,is_active,version,created_at,updated_at FROM employee_resources WHERE id=$id;"; command.Parameters.AddWithValue("$id",id); await using var reader=await command.ExecuteReaderAsync(token); return await reader.ReadAsync(token)?ReadResource(reader):null; }
    private static EmployeeResource ReadResource(SqliteDataReader r) => new(r.GetString(0),r.GetString(1),r.GetString(2),r.GetString(3),r.IsDBNull(10)?null:r.GetString(10),r.GetString(4),r.GetString(5),JsonSerializer.Deserialize<string[]>(r.GetString(6))??[],r.IsDBNull(7)?string.Empty:r.GetString(7),r.IsDBNull(8)?null:r.GetString(8),r.IsDBNull(9)?null:r.GetString(9),r.GetInt32(11)==1,r.GetInt32(12),ParseInstant(r.GetString(13)),ParseInstant(r.GetString(14)));
