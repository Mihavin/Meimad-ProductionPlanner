...
Machine's dialect (section 5) whenever the assigned Machine can deliver them: when its CNC
connection in Setup is enabled with a DPRNT source other than `NONE` (TCP, FILE or FTP), or
when Server Verification is enabled. The sequence variable of the cycle block is the
event-sequence mapping of the Machine's verification configuration when one exists, enabled
or not; without a configuration the dialect default is used (`#10504` on Haas NGC, `#504` on
FANUC and Mazak, `VC5` on Okuma OSP) and the package manifest says so
(`partCountingVariableFromConfiguration: false`). Only a Machine without a DPRNT connection
gets both markers silently removed; the template stays valid, and when the connection is
configured later the same released NC starts counting without being re-posted. Server
Verification adds the hook and the Offset Loader; it is not required for part counting.
...
- The event context block is emitted for verification-enabled and verification-disabled
  Machines alike; the hook only when verification is enabled; the cycle blocks whenever the
  Machine's CNC connection reads DPRNT (or verification is enabled).
- The generated Offset Loader prints its own context line (`MEIMAD/V/2/CONTEXT/PACKAGE/...`
  `/OFFSETRELEASE/<token>/...`) before the challenge call - on FANUC and Mazak inside its own
  `POPEN`/`PCLOS` pair - so the DPRNT log names the package and Offset Loader release before
  the `OLC` event that the protected challenge subprogram prints.
- The protected subprograms are generated by the Server per Machine in its dialect:
  `GET /api/v1/machines/{machineId}/verification-macros` returns a zip with O9001 (challenge),
  O9002 (verify), O9003 (finalizer) and a README (`?format=json` for the texts). Haas NGC
  reproduces the commissioned V10 macros; FANUC and Mazak use `POPEN`/`PCLOS` around every
  DPRNT and a `#3006` message stop for the operator's code; Okuma OSP is one `MEIMAD.SUB`
  User Task 2 library with PUT/WRITE C, `CALL`, `RTS` and an `M00` prompt. Every package other
  than Haas is uncommissioned on a physical control (OD-036).
...
Whatever the dialect, the hook line disappears and the Offset Loader comment resolves to
`NOT_APPLICABLE`; the context line is still printed. The cycle blocks stay when the Machine
has an enabled DPRNT connection (section 3); the example below is a Machine without one:
...
`#500-#999` (for example nonce `#501`, response `#500`, state `#502`, release token `#503`,
sequence `#504`). The generated hook and cycle blocks are valid custom macro B. The protected
O9001–O9003 subprograms are generated per Machine (`verification-macros`): each DPRNT sits in
its own `POPEN`/`PCLOS` pair, the verify program stops with `#3006=1 (MEIMAD CODE TO #500 THEN
START)` and the operator enters the code into the response variable on the macro-variable
screen before cycle start. They have not run on a FANUC control yet: commission them with
verification disabled before enabling it.
...
generated syntax is the FANUC macro B form without `G103`; the protected subprograms are the
FANUC rendering with the `MAZAK-<number>` event tag and must be commissioned on the Matrix
...
- The protected subprograms `O9001`–`O9003` (challenge, verify, finalizer) are generated as
  one User Task 2 library `MEIMAD.SUB` (`verification-macros`): PUT/WRITE C event lines,
  `CALL Onnnn PA=...` between the programs, `RTS` returns, `IF [...] Nname` branches, scratch
  values in `VC190-VC199`, a rolling nonce instead of a clock, and an `M00` prompt after which
  the operator enters the code into the response variable. They are commissioned on the
  Genos L200E-M before verification is enabled; the statement spellings are the ones the
  Server already emits and are confirmed on the control (OD-036).
