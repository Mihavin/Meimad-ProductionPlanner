...
The implemented model has no `ProductionBench` entity: its concrete production context is the existing `BatchOperation`, created from an existing `CaseOperation` and placed through an existing `MachineAssignment`. The proposed Production Run architecture generalizes this boundary without creating a second Machine, Case Operation, Batch Operation, or Production Batch model: Production Run becomes the physical assignment/session aggregate.
...
- In the implemented model, `ProductionBatch`, `BatchOperation`, `MachineAssignmentService`, and `SqliteMachineAssignmentRepository` are the production, scheduling, and execution boundary. In the proposed target, the same assignment services move whole Production Runs, while Batch Operations retain quantity and dependency ownership. Start remains the transactional pinning point.
...
Schema v38 separates parse-once release analysis from Machine evaluation. The analysis normalizes feed-motion seconds, rapid distance in millimetres, tool-change count, dwell, units, warnings, unsupported constructs, parser version, and confidence.

Since 0.1.123 the analysis comes from the **NC engine** (`NcEngineProgramAnalyzer`, parser version `nc-engine/<upstream version>+m<adapter revision>`, currently `nc-engine/0.17.0+m2`): the interpreter of the Chevalier NC viewer, vendored unmodified in `third_party/chevalier-nc-viewer` and hosted in a ClearScript V8 engine by `shared/Meimad.Planner.NcEngine`. It simulates the program instead of scanning lines: Custom Macro B / Haas macros and loops, subprogram calls (mill: by the interpreter; lathe: inlined by the adapter from the release folder or the machine's program-memory folder), canned cycles (G81-G89, G73/G74/G76, FANUC lathe G70-G76), feed per revolution with constant surface speed, inverse-time and rotary feeds, work offsets, tilted planes and TCP. `[[MEIMAD:KEY]]` placeholder lines, legacy V1 verification blocks and print statements (`DPRNT`, `PUT`, `WRITE`) are turned into comments before interpretation, keeping line numbers. The engine machine is chosen in this order: the **NC viewer machine** configured on every active Machine that supports the release's Postprocessor (Setup, `ncViewerMachine`; Machines left on auto-detect do not count as a disagreement), else an explicit `(MACHINE: ...)` comment, else detection from the program (lathe or mill, machine keywords and codes) refined by the shared NC dialect: a detected machine whose control reads the dialect is kept, otherwise the dialect's machine is used (`HAAS_NGC` → Haas UMC-500 / Haas ST-25Y, `FANUC_MACRO_B` → Doosan DVF 5000 / Chevalier FLC-200MC, `MAZAK_MATRIX_EIA` → Mazak Variaxis i-500, `OKUMA_OSP` → Okuma Genos L200E-M for lathes). Machine definitions with a Meimad **translation** (Okuma OSP lathe: LAP cycles, compound thread and grooving cycles, `CALL`/`RTS`, `VC` variables, named labels, `G94`/`G95`, `G04 F`, six-digit `T`; Haas lathe: one-block G71-G76 and G90/G92/G94 cycles; Mazak Variaxis: A tilt interpreted as B) are rewritten for the interpreter first, with every row mapped back to the original program. An `OKUMA_OSP` program interpreted without the Okuma translation (a mill, or another machine selected) keeps confidence `LOW` with `OKUMA_OSP_SYNTAX`. The FANUC lathe interpreter does not implement `G04`, so the adapter comments out dwell-only lathe blocks and counts their time from the program text.

Rapid moves (G0, G28/G53 returns, tool-change retracts, lathe G30) contribute distance, not time; tool changes contribute a count; `NcCycleTimeEstimator` then applies each Machine's configured rapid rate, tool-change time and time factor as before. The engine's own cycle (from its machine definitions) is shown only in the NC viewer. Confidence is `LOW` when any motion has no usable feed or spindle speed, the engine hit a resource limit, or OSP syntax was not translated; `MEDIUM` when the engine reported program issues (for example travel limits against placeholder work offsets); otherwise `HIGH`. If the engine cannot run (missing files, timeout after three minutes, heap limit), the release falls back to the former line parser `NcProgramParser` (version `1.0.0`: modal G0-G3, F, T/M6, G4, units, G90/G91, planes; macros, cycles and rotary motion lower its confidence) with a warning, and the release is still published.

Analyses are append-only and keyed by release and parser version; the newest analysis of a release is its current one, both in the Operation G-code catalog and in `effective_batch_operation_nc_estimates`. After startup `NcAnalysisBackfillService` gives every existing release without an analysis of the current version one (one release at a time, 30 s after start, `GCode:NcEngineBackfillEnabled`), so older releases move to the engine estimate without re-release; the earlier `1.0.0` analysis stays as history. A release the engine cannot analyze keeps its existing analysis and is retried at the next start.
...
For a not-started Batch Operation, planning precedence is `manager override > valid selected/current compatible NC estimate > manual batch_operations.cycle_seconds`. The pure estimator accepts the manager-override tier, but no persisted override authoring field exists yet, so current repository projection supplies null for that tier. Planning Board and Timeline use the selected source while preserving the manual snapshot. The Windows release history shows each calculated Machine-specific cycle per part, while the Planning Board card shows the assigned operation's selected per-part cycle and source. Started work does not switch duration source. Parser or Machine-input failure returns an unavailable estimate and falls back to manual timing; it never changes G-code readiness.
...
## Manufacturing Program generalization gate

The generalization is implemented according to [Production Run and multi-output Manufacturing Program architecture](production-run-architecture.md). Existing immutable release IDs and files are preserved; migrated revisions have one output with `quantityPerCycle = 1`, while combined revisions own immutable multi-output recipes. Production Run readiness is hierarchical at run, program, and output level. Tooling evaluates the distinct-tool union and identifies tool/pocket conflicts without plan mutation. Material remains output-Batch-specific, releases are explicit when ambiguous, and Start pins the exact revision, G-code/tool-table release IDs, and hashes.
