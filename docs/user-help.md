...
**View NC file** from the Tool Room or Setup queue, or from a Machine-assigned operation on the Planning Board, opens the program with this tool table: the viewer's tool table and the 3D simulation use the measured diameters and lengths and the cutter shapes, the status bar names the Tool Room table version, and tools you have not described keep the values read from the program.
