...
- Protected temporary variables are supported only for the future setup-verification challenge/response handshake after the Machine-specific protected-program technical spike is proven. They are not retained as Setup/Production state and their O-number/custom-code/variable mapping is configuration, not business logic.
...
- The challenge/response algorithm-v1 reference folds six-digit nonce, Offset Loader token, NC identity, and a Server-derived/protected six-digit Machine key into a configurable 4-6 digit response using only bounded decimal arithmetic. It is test-vector tooling only until the real control independently reproduces the vectors, leading-zero behavior, cleanup, and failure alarm. No tablet or workflow endpoint currently calculates or exposes a response.
...
- Per-Machine protected-verification configuration is Server-owned and includes DPRINT transport, protected programs/custom alias, temporary variables, encrypted Machine secret, expected macro version, 4–6 response digits, timeout, and enablement. Public planning responses expose only `secretConfigured`; tablet contracts receive none of these sensitive details. This foundation does not claim the protected macro, active NC identity, or response algorithm is commissioned.
...
Implemented Server baseline: active Windows editors can register, bind/unbind, revoke, and rotate E-Ink devices; plaintext credentials are returned only when created or rotated. An active editor can publish immutable official job-package revisions, and device-scoped GET endpoints provide conditional version/status, structured Machine/package data, checksum-verified files, and time configuration. Schema v49 stores shared append-only workflow events and derives physical-tablet status from the latest run event. The separate ESP32 prototype compiles its bounded status client, retained revision behavior, deep-sleep policy, and guarded long-D4 `SEND_TO_QC` action. Official tool binding, automatic work-window enforcement, physical wake/current/input behavior, and shop-floor readability remain unverified. The `SEND_TO_QC` Server POST endpoint, authorization, eligibility/idempotent-response behavior, and end-to-end verification are not yet implemented; storage and read projection are implemented. Package approval roles/UI and retention, physical SD staging, local annotation persistence, and the remaining ESP32 acceptance work are incomplete.
