...
- Every newly approved CNC NC release is an immutable canonical template using `[[MEIMAD:<KEY>]]`. Required server-owned fields include Part/Operation names, Run/package/Machine/release identities, event context, optional Offset Loader resolution, and one standalone verification insertion point before executable code. The postprocessor supplies no authoritative Planner identity and needs no Server access. The Server assigns the immutable six-digit NC identity, preserves the uploaded bytes, and resolves only a generated package copy. Verification-enabled packages inject the configured hook and a uniquely bound Offset Loader; verification-disabled packages remove the hook and create no verification Offset Loader. Historical `(MEIMAD PACKAGE ... V1)` releases remain readable/buildable through a named compatibility parser but are not current postprocessor output.
