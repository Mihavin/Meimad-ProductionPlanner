...
Planning is allowed without released context. On the first Start, a MANUAL Machine disables only the G-code requirement; a CNC Machine requires one current release whose Postprocessor is explicitly supported by its assigned Machine. The Server pins the exact process, G-code, tool-table IDs and file hashes on the Batch Operation. Suspend/resume/finish preserve those pins; reset clears them. A later release never switches in-progress work. Missing, incompatible, or ambiguous G-code blocks Start with an explanation and never moves the assignment or modifies the file. Legacy operations with no managed process revision preserve their pre-v35 execution behavior. This contextual rule is not a global stored `Operation.IsReady` flag; tool offsets, normalized tool comparison, estimates, and authoritative material readiness remain later work.

Schema v36 parses every new released tool-table CSV/JSON into immutable tool rows: identifier, description, required/optional state, magazine-position requirement, active state, and optional position label. `RequiredToolCount` is the count of distinct case-insensitive identifiers that are active, required, and consume a magazine position. Duplicate rows, optional tools, inactive/history rows, external tools, and the highest numeric T value do not inflate it. The Planning Board evaluates the active process against the assigned Machine's `UsableToolPositions` on every read. A mismatch remains visibly planned but is not tool-capacity-ready, disables first Start in the Windows board, and is rejected again by the Server's Start transaction with required/available counts. Assignment, Machine capacity, and active process/tool-table changes are reflected without rewriting the plan. The Server never edits the program, drops tools, splits work, or moves an assignment. Tool offsets, full tool identity/life comparison, estimates, and authoritative material readiness remain later work.
...
- A Machine has explicit `CNC_GCODE`/`MANUAL` execution mode, supported-Postprocessor mappings, usable tool capacity, and explicitly unit-labelled estimator timing parameters. Start-time G-code compatibility is enforced for operations that have entered managed process-revision history; broader readiness and estimation remain later work.
...
- A Server-local Kitaron connector page at `/kitaron-setup/`. Only loopback requests may use it. Steps 1-3 manage encrypted connection settings, metadata detection, and an optimistic Draft/Ready field mapping. Step 4 shows one-way synchronization status and permits a manual run; Ready mappings also run periodically. SQL Server is always opened with read intent. Schema v31 stores Case Components and component sync counters/links. Each run atomically creates or refreshes connector-owned Cases, canonical `TSubOrder` Orders, reusable Case Operations, and every valid direct BOM edge returned by `TTreeNodes`, including BOM-only roots absent from the planning view; duplicate source edges reuse one relationship. `TSubOrder.StopProduction = 1` is Planner `cancelled`, including cancelled rows omitted from the planning view. Child demand equals parent demand multiplied by every quantity along its path and is preview-only. Sub-cases receive no automatic Order or Batch. Matching manual records are linked but never overwritten; stale links are repaired, missing imported component edges are deactivated, legacy parent routes skip conflicting edges with warnings, failures roll back, and no Kitaron mutation, allocation, Machine assignment, backlog entry, or Timeline position is created.
