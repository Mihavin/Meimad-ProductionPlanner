...
- Per-Machine protected-verification configuration is Server-owned and includes DPRINT transport, distinct challenge/verify/finalizer protected programs, optional custom alias, four temporary handshake variables, and one persistent increment-only event-sequence variable, encrypted Machine secret, expected macro version, 4–6 response digits, timeout, and enablement. Public planning responses expose only `secretConfigured`; tablet contracts receive none of these sensitive details. For the enabled TabletID bound to the session Machine, the existing status route derives only the fixed-width response in memory while the session is pending, current, enabled, and unexpired. It omits the code otherwise and never returns the nonce, secret/key, variable mapping, or algorithm details. Strict protected-macro success/failure DPRINT ingestion requires the exact current NC identity, Offset Loader release token, and nonce before Server workflow resolution. This prevents a delayed old result from resolving a newer challenge. Quarantined macro versions 3-5 do not emit that complete result evidence and are incompatible with the hardened Server contract. Physical CNC operator input, alarms, cleanup, protected access, Reset/power-cycle behavior, and the alarm-before-cutting interlock remain uncommissioned.
...
Schema v60 extends the Machine verification configuration with a distinct
protected finalizer program and one persistent increment-only event-sequence
variable. New saves enforce three distinct O9xxx programs, an M109-valid response
variable, and a distinct `#10000-#10999` sequence mapping. Macro v6 is a
separately generated no-motion candidate using those mappings. Structural tests
do not replace internal controls-engineering review or physical proof. The selected
sequence design is `PERSISTENT_COUNTER`: initialize it once to the recorded
positive value 1, use one increment-only domain for OLC/SVS/SVF/CST/CEN, never
wrap or silently reseed it, and fail closed when unset, invalid, decreased, or
exhausted.
