...
- When an obsolete aggregate or non-Kitaron Order is referenced by a Production Run whose structure is already locked, synchronization retains that Order and its Batch solely as immutable production history, counts it as a warning, and continues applying current canonical Kitaron demand. Never-started dependent Batches remain eligible for complete cleanup. An `in_production` or completed Batch cannot be physically deleted.
