...
- Timeline is one canonical read-only projection, not a separate view or layer per mode. Each Machine Assignment produces one operation block. `Forward` calculates its earliest feasible consequence, `Backward` latest-fits that same assignment before the earliest Work Finish Date among its linked Orders, and `Manual` retains the planner-authored Machine/backlog placement while calculating its valid visible consequence. Mixed modes share the same Machine lanes and dependency graph; equal-date backward contention uses shorter duration and then naturally smaller Order Number. No mode stores calculated start/end dates, creates another assignment/block, or changes backlog order. Missing, outside-horizon, or infeasible backward deadlines return explained conflicts.
