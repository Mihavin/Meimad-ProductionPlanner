...
- A staged legacy Excel importer. Preview is read-only and shows workbook sheets, editable source-column mappings, detected Machine sections, source values/formula provenance, existing-entity candidates, and blocking/warning issues. The operator explicitly maps or skips unclear rows, maps each selected section to an active Machine, and supplies every required Case, Order, route Operation, Batch allocation, and cross-type override decision. Commit requires Edit Mode, at least one create/assignment action, and is all-or-nothing. Imported assignments append after existing backlog rows in source order and start in `manual`; import never invents route/timing data, moves an assigned operation, schedules dates, or rewrites the workbook.
