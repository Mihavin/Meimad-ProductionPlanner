...
- Schema v38 preserves append-only per-Release/per-Machine calculation inputs and results. The Windows Operation release history displays every available Machine-specific calculated cycle time per part, and each Planning Board operation card displays the selected per-part cycle with an explicit NC/manual/override source label. For not-started work, planning cycle precedence is a valid selected/current compatible NC estimate, then the unchanged manual Batch Operation cycle snapshot. There is no implemented manager-override field; that layer and actual-time calibration remain future work. Started work does not change duration source, and actual time never overwrites an estimate.
