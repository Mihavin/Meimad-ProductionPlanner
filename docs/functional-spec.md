...
- Haas polling runs on the Server and keeps connectivity separate from `WAITING -> SETUP -> PRODUCTION -> COMPLETED` Bench execution state.
- The Part identity is the configured header value parsed from the NC program present on the Haas. Original filename and O-number remain separate informational/locator fields and never identify the Part.
...
- CNC communication is owned exclusively by the Server-side connection manager. Machine model/type never selects a protocol; the Machine's primary `MachineConnection.adapterType` does. `HAAS_NGC` is implemented, while registered MTConnect, OPC UA, and Custom choices remain visibly unsupported and cannot be enabled.
- Every adapter emits the same normalized snapshot and capability/runtime-health model. Bench automation consumes that normalized model only. Mandatory MDC loss is `OFFLINE`; optional Haas program-access loss is `DEGRADED` and must not discard valid state, variable, or counter data. Retained field values carry their read time and stale flag.
