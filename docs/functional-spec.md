...
- A Machine has explicit `CNC_GCODE`/`MANUAL` execution mode, supported-Postprocessor mappings, usable tool capacity, and explicitly unit-labelled estimator timing parameters. These values configure future readiness/estimation; Task 2 does not yet enforce G-code readiness on Start.
