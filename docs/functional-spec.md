...
- A Server-local Kitaron connector page at `/kitaron-setup/`. Only loopback requests may use it. Steps 1-3 manage encrypted connection settings, metadata detection, and an optimistic Draft/Ready field mapping. Step 4 shows one-way synchronization status and permits a manual run; Ready mappings also run periodically. SQL Server is always opened with read intent. Schema v31 stores Case Components and component sync counters/links. Each run atomically creates or refreshes connector-owned Cases, canonical `TSubOrder` Orders, reusable Case Operations, and the reachable multi-level direct BOM from `TTreeNodes`. `TSubOrder.StopProduction = 1` is Planner `cancelled`, including cancelled rows omitted from the planning view. Child demand equals parent demand multiplied by every quantity along its path and is preview-only. Sub-cases receive no automatic Order or Batch. Matching manual records are linked but never overwritten; missing imported component edges are deactivated, failures roll back, and no Kitaron mutation, allocation, Machine assignment, backlog entry, or Timeline position is created.
