...
- Every newly approved CNC NC release is an immutable source template containing exactly one `(MEIMAD PACKAGE VERIFY V1 NCID=xxxxxx)` marker before its first executable block. The six-digit NC identity remains globally unique and bound to that exact release. An optional cycle-instrumentation pair is exactly `(MEIMAD PACKAGE CYCLE START V1)` and `(MEIMAD PACKAGE CYCLE END V1)`, in that order and at most once each. The Server preserves the uploaded bytes and resolves these markers only during an atomic Machine-specific Production Package build. A verification-enabled package receives the configured verification hook and commissioned CST/CEN content; a verification-disabled package has the markers removed and contains no active Server-verification code.
