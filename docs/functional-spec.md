...
- Haas polling runs on the Server and uses the explicitly selected read provider, `MDC` or read-only `MTCONNECT`; it keeps connectivity separate from `WAITING -> SETUP -> PRODUCTION -> COMPLETED` Bench execution state. Existing saved configurations default to MDC until a planner explicitly selects and saves MTConnect.
...
- Cycle Start telemetry never changes Setup to Production. Only the configured binary macro's observed `0 -> 1` transition starts Production, enables counting, captures the selected counter baseline, and creates one idempotent `BenchProductionStarted` event.
- Counter increments are immutable Part events only while Production counting is enabled. A counter decrease creates a reset event, establishes the next baseline, and never subtracts produced quantity.
- A confirmed Tool Table transfer may reset only the configured macro to zero. The Server audits the attempt before the write, reads the value back, and treats an unconfirmed `1 -> 0` as an anomaly while preserving prior intervals/history.
...
- Every adapter emits the same normalized snapshot and capability/runtime-health model. Bench automation consumes that normalized model only. Loss of the selected Haas read provider is `OFFLINE`; unavailable optional program-header access, counter data, or a missing/nonbinary MTConnect macro mapping is `DEGRADED` and must not discard other valid observations. MTConnect never writes a CNC variable. Automation requires the current observation to carry the configured variable number, a read timestamp, `stale: false`, and exactly `0` or `1`; it never substitutes a previous value. Part attribution likewise requires a current non-stale counter. Retained field values carry their read time and stale flag.
- The TV dashboard uses the current non-stale Haas Production Mode variable for an in-progress operation's phase: `0` displays Setup and `1` displays Production. Elapsed timing estimates completion only within that live phase; it does not override the variable.
