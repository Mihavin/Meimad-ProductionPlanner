...
- A Server-local Kitaron connection page at `/kitaron-setup/`. Only loopback requests may read or change the connection. It manages host, port, database, schema/view, username, encrypted password, and periodic read-only check interval, and shows the last connection-test status plus detected source columns. Passwords never leave the Server. The connection tester uses SQL Server read intent and schema-only `SELECT TOP (0)`; it never issues `INSERT`, `UPDATE`, or `DELETE`. Periodic checks verify connectivity only. Case/Order/Operation synchronization remains disabled pending the explicit Operation-ownership decision.
