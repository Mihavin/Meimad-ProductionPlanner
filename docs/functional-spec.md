...
- A Server-local Kitaron connector page at `/kitaron-setup/`. Only loopback requests may use it. Steps 1-3 manage encrypted connection settings, metadata detection, and an optimistic Draft/Ready field mapping. Step 4 shows one-way synchronization status and permits a manual run; Ready mappings also run periodically. SQL Server is always opened with read intent and synchronization issues only a selected-column `SELECT`. Schema v30 stores sync state and stable source links. Each run atomically creates or refreshes connector-owned Cases, Orders, and reusable Case Operations; matching manual records are linked but never overwritten. Repeated rows are idempotent, disappeared source rows are retained, and failures roll back. Synchronization never writes to Kitaron and never creates Production Batches, Batch Operations, allocations, Machine assignments, backlog entries, or Timeline positions.
