...
- A staged legacy Excel importer. Its primary Windows page asks only for the workbook and worksheet, applies the defined Cases/Orders/Batches mapping, and shows **Cases in the Case Pool → related Orders → Batches in the Pool → Machine assignment → Review/commit**. The old five-tab outcome/column/Machine/pattern wizard is hidden from the normal workflow. These are focused views of one read-only staged workbook, not separate imports: the final Commit validates every approved selection atomically. Preview still retains workbook metadata, source values/formula provenance, existing-entity candidates, and scoped blocking/warning issues for server validation. Invalid mapped Order dates/quantities and missing required row identifiers remain blocking. The operator can create Cases/Orders, create a Batch from an existing Case route into the unassigned Planning Board pool, assign one selected created/existing Batch Operation to a Machine, or Skip.
