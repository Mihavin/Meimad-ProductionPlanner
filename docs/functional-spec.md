...
The Windows client supports Machine and Production Batch editing through Server version-checked APIs. Batch editing changes Batch Number, planned quantity, and its complete balanced allocation set without recreating its instantiated route or changing operation execution/assignment state. Guarded deletion is available for Cases, Case Operations, Orders, Production Batches, and Machines. A confirmed Production Batch deletion under active Edit Mode first rejects any Batch referenced by a started or completed structure-locked Production Run with `409 delete_blocked`; this preflight occurs before mutation and preserves all production evidence. For a never-started Batch, deletion atomically removes its owned assignments, pauses/execution records, allocation/Operation rows, assignment overrides, and package database records, then compacts affected Machine backlogs and recomputes linked Orders. It does not delete external Case folders, images, original engineering content, or physical package files.
...
### Production Batch cancellation

The Windows Batch workspace implements an explicit **Cancel production** action for an Edit Mode holder. Cancellation is atomic and terminal for that production attempt: the Batch and all of its Batch Operations, Production Runs, and Run Programs become cancelled; active Machine assignments and material reservations are released; modern Run output/completed-cycle projections and the legacy Haas bench produced quantity are reset to `0`; active pause/bench intervals are closed; and affected Machine backlogs and Order lifecycle projections are recalculated. Immutable CNC cycle observations, workflow events, and the structured cancellation audit remain stored as evidence. A Production Run producing outputs for more than one Batch cannot be cancelled through this single-Batch action because coupled outputs remain atomic.
...
Client and Server are released as a version pair. The Server distributes the client installer of its own version (`GET /api/v1/client-installer`), and the client checks the pair once per session after its first successful health check: a newer client package on the Server is downloaded, checksum-verified, and installed automatically after the user is told "New version available, the client will be installed" (the client closes and restarts itself; the per-machine install still shows the UAC prompt); a client newer than the Server's package is never downgraded and only shows an attention notice, as does a mismatch on a Server without a bundled installer.
...
- A Server-local Kitaron connector page at `/kitaron-setup/`. Only loopback requests may use it. Steps 1-3 manage encrypted connection settings, metadata detection, and an optimistic Draft/Ready field mapping, including a `material_orders` group. Step 4 shows one-way synchronization status and permits a manual run; Ready mappings also run periodically. SQL Server is always opened with read intent. A Case linked by synchronization is Kitaron-managed: Kitaron is the sole authority for its imported master fields and complete Order set. The Windows client exposes those fields and current Orders as read-only, and the Server rejects Case update/delete and Order create/update/delete attempts with `409 kitaron_managed_read_only`. Manually created, unlinked Cases and their Orders retain normal edit/delete behavior. Each run selects parts from the planning view and imported BOM, then reads every canonical `TSubOrder` delivery row for those parts regardless of planning-view Order-number coverage; exact duplicates are removed, while every distinct row keeps its quantity, supply date, unit Price from `TSubOrder.PriceInCurr` in the Kitaron order currency, and `<OrderNumber>/<RecordID>` reference. Kitaron Order status is projected as `active` when open, `inactive` when any recognized `TSubOrder` row or parent `TOrder` closure/completion flag is set, and `cancelled` when `TSubOrder.StopProduction` is set; cancellation takes precedence over closure. Planner Orders absent from the current Kitaron set are removed. When such an Order supplies direct or derived demand, its dependent Production Batch is intentionally removed through the normal complete Batch-graph cleanup so the planner can recreate it manually from current Kitaron demand. Orders, allocations, Batch Operations, assignments, runs, packages, and related operational rows are removed together in the same atomic sync; an unexpected protected reference rolls everything back. During migration, an exact plain-number row may receive the canonical reference without being duplicated. The same transaction creates or refreshes connector-owned Cases, reusable Case Operations, and every valid direct BOM edge returned by `TTreeNodes`. Schema v41 also imports `TBuyRow` raw-material purchase lines plus the latest matching `TAppCostOfferBySupplier` delivery approval into an advisory register. Historical Kitaron received quantities and approvals never create a locally verified receipt or reservation and never satisfy material readiness. Stale links are repaired, missing imported component edges are deactivated, failures roll back, and no Kitaron mutation exists.

- Canonical Order scope is part-based, not Order-view-based: for every Kitaron part discovered through the planning view or the imported `TTreeNodes` BOM, synchronization reads every `TSubOrder` row for that part. An Order does not need to appear in the planning view, so open, closed/supplied, and cancelled delivery rows remain in the authoritative Case Order set. The known Kitaron test Order `הזמנה לדוגמא 1` is explicitly excluded at the complete source-row boundary, so its dummy Case and Operation data cannot claim or modify a same-number manually managed Planner Case.
- When an obsolete aggregate or non-Kitaron Order is referenced by a Production Run whose structure is already locked, synchronization retains that Order and its Batch solely as immutable production history, counts it as a warning, and continues applying current canonical Kitaron demand. Schema v67 marks that retained row `kitaron_history_only`; it remains available by stable Order ID and through its Batch/Run history but is excluded from the Case's current Orders list, demand projection, and Batch-allocation choices. For every Kitaron-managed Case, current demand is positively selected by a current Order source link; any unlinked legacy row is likewise excluded even if an older installation failed to classify it as history. Kitaron ownership applies to the complete Order collection under the Case, so such an unlinked row cannot be edited or deleted, recreated through legacy import, or assigned to a new/edited Production Batch. Every successful synchronization also removes unlinked orphans below durable Kitaron-managed Cases, even when a partial source snapshot omits that Case; linked canonical Orders for an omitted Case are preserved because their absence was not observed authoritatively. Dependent Batches without structure-locked Production Run history remain eligible for complete cleanup, including test-plan Batches whose projected status had advanced. A Batch referenced by a structure-locked Production Run cannot be physically deleted.
- The connector page shows `orders.status` and `orders.price` as locked connector-managed mappings. They cannot be disabled or redirected to another Kitaron field; configurable planning-view mappings remain separately editable.
...
- Every newly approved CNC NC release is an immutable canonical template using `[[MEIMAD:<KEY>]]`. The release gate checks only that exactly one standalone `[[MEIMAD:VERIFICATION_HOOK]]` occurs before executable code; it does not interpret ordinary O/G/M blocks or unrelated `G65 P...` machining calls. Other optional Server-owned metadata/event placeholders are resolved and checked when a concrete Production Package is built. The postprocessor supplies no authoritative Planner identity and needs no Server access. The Server assigns the immutable six-digit NC identity, preserves the uploaded bytes, and resolves only a generated package copy. Verification-enabled packages inject the configured hook and a uniquely bound Offset Loader; verification-disabled packages remove the hook and create no verification Offset Loader. Historical `(MEIMAD PACKAGE ... V1)` releases remain readable/buildable through a named compatibility parser but are not current postprocessor output.
- The current secretless challenge/response binds the six-digit nonce, Offset Loader token, NC identity, exact Run, and recognized configured Machine context without a Machine Secret or derived Machine key. Earlier public-key arithmetic vectors remain historical controller evidence, not a credential or production-interlock claim. Schema v52/v63 atomically creates an untimed ARMED session from a valid current Offset Loader event, starts the timeout only on the first exact NC request, and supersedes the session when a newer Offset Loader is selected. The assigned tablet derives the response in memory under the fail-closed rules below.
...
- After a Server-recorded `QC_PASS`, the assigned planned Production Run Program enters production through the first valid Haas DPRINT `CYCLE_START` (`CST`): the Server atomically changes the Run to `IN_PROGRESS`, the resolved Program to `ACTIVE`, its Outputs to `IN_PRODUCTION`, locks the Run structure, and retains the START as the open attempt. Manual Start remains only for Machines without a configured CNC connection and non-CNC work. A `CYCLE_END` (`CEN`) completes one cycle only when it is the immediately consecutive event from the same Machine source and matches that open Run/program attempt. Program identity is mandatory and must match the Server-resolved target; Run identity remains optional but must match when supplied. Missing identity fails closed before any workflow or quantity write. The END workflow event, idempotent cycle record, all coupled-output quantity changes, parent status projection, and structured audit commit atomically. Retries do not count twice, and a source identifier reused for a conflicting cycle type is rejected as an anomaly. Controller part counters remain diagnostic and cannot advance official quantity.
...
- Schema v58 preserves each sequenced production attempt as immutable raw evidence. START and its `COMPLETED` or `INTERRUPTED` boundary retain separate Server receipt times, optional Machine timestamps, source event identities, and sequences. An outcome-less START is `OPEN`; an orphan or nonconsecutive END never becomes a completion. Calculated duration is not stored. After the first completed attempt, the live Timeline derives the arithmetic mean of all `COMPLETED` attempt durations in that current Production Run Program and forecasts only its remaining cycles from the current projection time. Every later completion recalculates the series mean. Interrupted, open, orphaned, duplicate, and unmatched events are excluded. Before the first completed measurement, the configured planning cycle time remains the fallback.
...
- Per-Machine setup-verification configuration is Server-owned and includes DPRINT transport, protected challenge/verify/finalizer programs, temporary handshake-variable mappings, an evidence-only event-sequence variable, expected macro version, response width, timeout, and enablement. CNC Machine identity is only `MachineID` plus the configured fixed IP and controller MAC; there is no Machine Secret or replacement credential. An exact Offset Loader completion creates untimed `ARMED`; the assigned tablet may display its response. The first exact NC-start `SVR` changes it to `PENDING` and starts the timeout. Success/failure requires the exact Run, Machine, NC identity, Offset Loader release, and nonce. Success is reused until a new Offset Loader supersedes it. Sequence reset/gap/duplicate/out-of-order evidence never establishes or invalidates identity or verification by itself. Physical controller behavior remains a commissioning requirement.
...
- CNC communication is owned exclusively by the Server-side connection manager. Machine model/type never selects a protocol; the Machine's primary `MachineConnection.adapterType` does. `HAAS_NGC` is implemented and may explicitly select MDC, MTConnect, or DPRNT-only as its read provider. `FANUC_FOCAS` is implemented as a read-only FOCAS 2 adapter (controller state, executing program, part counter, spindle/feed, alarms) that requires the FANUC-licensed FOCAS library on the Server. Every adapter reads DPRNT output from the same source choice: the controller's TCP port or a serial-to-Ethernet bridge (`TCP`), or a controller-written print file on a share (`FILE`, for example Mazak Matrix `print.txt`) that the Server may empty at each new Offset Loader completion, after each read, or never. The separate vendor-neutral `MTCONNECT`, OPC UA, and Custom adapter choices remain visibly unsupported and cannot be enabled.
...
Schema v63 retains the distinct protected finalizer and event-sequence mapping,
but sequence is evidence only. V10 automatically recovers an unset, invalid, or
exhausted sequence value and the Server records discontinuities without blocking
identity, verification, workflow, or an otherwise exact cycle pair.
Schema v61 additionally requires persistent global storage for every value that
crosses protected-program calls and canonicalizes Haas legacy aliases before
collision checks (`#500` and `#10500` identify the same storage).
Quarantined macro versions 1-5 may be retained only in disabled configuration;
the Server refuses to enable them.
...
Production Package creation offers `MEASURED` and `MANUAL_DUMMY` tool-offset modes. `MANUAL_DUMMY` requires assigned-Machine configuration, omits the measured Tool Table artifact, and records setupist manual-entry responsibility in the immutable manifest. A CNC Manual/Dummy package also requires enabled Server Verification: it always produces the exact package/Run/Machine/NC-bound verification-only Offset Loader and normal verification/setup hooks, with no measured offset payload or offset-write commands. It fails clearly instead of silently creating a package without that loader. Schema v65 initially enables existing Machine-number records 10, 14, and 15 through configuration rows; domain code never branches on those numbers.

## 10. TV Dashboard

The TV Dashboard is a read-only web surface suitable for fullscreen/kiosk display. Its normal display is a compact dark factory board with one horizontal band per active, display-enabled Machine. Each band shows Machine number/name and connection dot, current Part Number, Batch and Operation, Operation name, large textual execution state, the authoritative Production Run output count against target, the current completed-cycle series average when available, and a thin progress bar. Idle rows explicitly say `No current operation`. Part previews, host names, Server URLs, debug text, summaries, configuration, and controls are absent. Color is never the only state signal.

Implemented baseline: the Server serves a dependency-free kiosk page and `GET /api/v1/tv-dashboard`. The browser fits the compact Machine bands to the viewport, conditionally refreshes every 15 seconds by default, and retains the last rendered status snapshot when refresh fails. It contains no scrolling, edit controls, or Edit Mode calls. The Server projection remains richer and authoritative; the kiosk only renders its Machine/job/status result. Authentication, offline-display telemetry, physical target-screen/read-distance acceptance, and kiosk deployment management remain TBD.

### Role preparation queues and Production Package foundation

The Windows client provides dedicated NC Creator, Tool Room Manager, Setup, and existing QC queue views using one common list/detail/refresh interaction pattern. Preparation queue membership is a read-only Server projection and is never an editable status or a client decision. Unassigned Operations appear in none of the preparation queues, and no preparation view can select or change a Machine.

For a CNC G-code Machine, `PROGRAMMING_PENDING` means a concrete Machine assignment exists but the active process has no effective current G-code release satisfying the Machine/Postprocessor and explicit-selection rules. A MANUAL Machine does not require G-code. Once the NC gate is satisfied, `TOOL_PREPARATION_PENDING` covers both incomplete Tool Table/capacity/exact Tool Offset prerequisites and the absence of one valid current Production Package for the exact Operation and assigned Machine. Only successful atomic package activation moves the Operation to `SETUP_PENDING`; opening, downloading, copying, or exporting a package never changes workflow. `SEND_TO_QC`, `QC_PASS`, or production events remove it from all preparation queues.

The Tool Room deliberately creates the Server-owned immutable package without a second approval. The Server records creator and Server time and stores verified artifacts plus a manifest under its configured package root. CNC with Server verification enabled contains runnable NC, current Tool Table, a newly generated package-specific Offset Loader, and manifest. CNC with verification disabled contains runnable NC, Tool Table, and manifest with all verification placeholders removed. MANUAL contains only applicable Tool Table/manual setup material and manifest. Connectivity controls only whether direct transfer is offered; file export remains available and verification policy is never silently downgraded. Machine, NC, Tool Table, or verification-content configuration changes make the pointer ineligible immediately; a later build preserves history and records supersession.
...
- MVP tablet communication is intentionally unauthenticated on the trusted factory LAN. TabletID is an identifier only; MAC is optional discovery/mapping metadata. No tablet token, key, password, certificate, session, or hidden credential exists.
- Server-observed last-seen, optional battery/firmware telemetry, and offline status must remain separate from planning data.

`SEND_TO_QC` is an operational workflow command, not a planning edit. The request contains only `event_type: SEND_TO_QC`; it cannot name a Machine, run, program, Operation, or timestamp. The Server resolves the enabled TabletID, resolves the bound Machine and eligible active Production Run, creates the timestamp, records an append-only event, advances the tablet workflow projection to `IN_QC`, and changes the status revision. It does not assign, reorder, start, suspend, complete, allocate, count cycles, publish files, or change official package content. Single Edit Mode is not required. A repeat for the same run while its tablet workflow is already `IN_QC` returns the original accepted event and timestamp without another state change or duplicate event.

Implemented Server baseline: active Windows editors can register, bind/unbind, enable, and disable E-Ink devices; registration returns TabletID and never creates or returns a tablet credential. An active editor can publish immutable official job-package revisions, and device-scoped GET endpoints provide conditional version/status, structured Machine/package data, checksum-verified files, and time configuration. Schema v49 stores shared append-only workflow events and derives physical-tablet status from the latest run event. Schema v54 and the TabletID-identified POST route resolve the tablet's current Run, enforce `IN_SETUP_RUN`, persist one `SEND_TO_QC` per inspection attempt, and return the original Server timestamp on retries without Edit Mode or planning/package mutation. Schema v55 permits a fresh send only after an immutable `QC_FAIL` returns the same Run to `IN_SETUP_RUN`. ESP32 firmware 0.1.3 compiles the centralized power policy: setup states remain awake with Wi-Fi off and no background poll; D1 performs bounded synchronization; QA and production sleep button-only; canonical post-QA `READY_FOR_PRODUCTION` adds a configurable 60-second one-shot refresh. The physical status payload projects the Run's released tool rows and firmware 0.1.6 renders them as paginated tool/description/pocket pages; automatic work-window enforcement, physical wake/current/input behavior, and shop-floor readability of those pages remain unverified. Package approval roles/UI and retention, physical SD staging, local annotation persistence, and the remaining ESP32 acceptance work are incomplete.
...
The firmware logs wake reason, available UTC time, battery voltage, and retained
policy state. Setup UI awake and Wi-Fi connectivity are independent: Wi-Fi is
off between explicit D1/service/send sessions, and D2/D4 page navigation remains
local. Post-QA timer, cold boot, Refresh, and send use bounded Wi-Fi only when
required. The radio is disabled after HTTP work and before panel refresh or
deep sleep. Retained state never replaces Server state. Clock synchronization,
configured work-window gating, and measured current/runtime behavior remain
unverified.
...
Implemented backup decision: the Server uses SQLite online backup into local staging, publishes timestamped files to a configurable folder, retains a configurable number of verified backups, checks integrity and foreign keys, and restores only to an isolated test database for verification. It never restores over the active database. The active Edit Mode holder can request and download a fresh verified backup through the factory-LAN HTTP API; this operation is audited and returns a SHA-256 checksum that the Windows client verifies. Human authentication, backup schedule, encryption, and disaster-recovery replacement remain open.
...
The Windows client and TV dashboard support English, Hebrew, and Russian. They select the browser/operating-system language on first use and provide an explicit language selector. The choice persists locally. Hebrew uses right-to-left window flow while identifiers, Part Numbers, Machine Numbers, dates, durations, and other technical values retain their required representation. Server API values, SQLite values, paths, imported source data, and user-entered master data are never translated. **Proposed (OD-039):** a picker that lists a fixed set of enumerated Server codes, such as employee roles, availability exceptions, holiday policies, weekdays, order statuses, dependency types, or Machine execution modes, shows a translated name for each code while the value it stores, sends, and validates remains the code. Codes that help text, validation messages, or controllers quote literally stay as written, for example NC dialects, DPRNT sources and clear policies, controller part-counter sources, and time-zone IDs.

Windows localization is event-driven. Each element is localized the first time layout gives it a size, which covers controls realized after a window loads: a tab page shown for the first time, item templates, Timeline blocks built in code, and dialogs. Selecting a tab runs one pass over that page after layout, so a page that was hidden during a language change catches up. A context menu is localized whenever it opens, and rapid language changes coalesce to one pass over each open window. A translated value stays observed, so a later bound status change is translated too; the element itself holds that observation, so it never keeps a closed window or a discarded item in memory. There is no recurring visual-tree localization scan while the client is idle, and persisting the language choice does not block the UI thread. Expensive Timeline and STEP drawings similarly coalesce layout/data changes, defer redraw while hidden, and redraw at most once when a dirty view becomes visible. The five-second Server refresh runs at background dispatcher priority, cannot overlap itself, and unchanged edit-session snapshots do not rebroadcast presentation state.

Messages composed at run time, including Server messages shown in the client, are translated from catalog templates sentence by sentence and line by line, so embedded names, numbers, and identifiers stay unchanged. Message boxes and file dialogs show their text, title, and file-type names in the selected language, and Hebrew message boxes read right to left; their buttons follow the Windows display language. The embedded NC viewer page follows the selected language for its interface text. The NC program, computed values, data tables, and form values are never translated, and Hebrew phrases are shown as right-to-left isolates inside the left-to-right viewer.
...
7. Tablet routes accept TabletID only, resolve assigned reads and `SEND_TO_QC` targets on the Server, and never accept a client-selected Machine or Run. TabletID provides no Windows planning authority.
