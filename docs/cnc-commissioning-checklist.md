...
| Approved protected variable mappings | `#10500-#10503` and persistent event counter `#10504` were collision-reviewed for the bounded VF-3SS macro-v6 attempt. The physical R2 cleanup ended with #10500-#10503 empty and #10504 = 20; v6 is quarantined after retaining the sixth M109 ASCII value during the failed attempt. |
| Expected protected-macro version | NOT RECORDED |
| Sequence persistence/reset rule | `PERSISTENT_COUNTER` selected: initialized once, increment across OLC/SVS/SVF/CST/CEN, never wrap/reseed, and fail closed at an invalid/decreased/exhausted value. VF-3SS engineering tests and the R2 attempt observed #10504 through value 20, but full controlled adjacency/reboot/wrap commissioning remains incomplete. |
...
| 13 | Sequence behavior across normal cycles, RESET, E-stop, program restart, controller reboot, and wraparound is recorded. Server gaps/out-of-order/duplicates remain evidence, never invented events. | FAIL | Earlier v3-v5 findings remain. The 2026-08-30 R2 attempt additionally advanced persistent #10504 through 20 and retained the sixth M109 ASCII value in #10500 after alarm 903. Multiple operator attempts prevented isolation of the reported M30 observation; Server evidence retained OLC/SVF facts and no SVS. Macro v6 is quarantined. Macro v7 has desk-only cleanup regression coverage and no physical acceptance. | [Code audit](cnc-verification-code-audit-2026-08-27.md); [bounded retest record](haas-bounded-retest-after-internal-approval.md). |
