...
- **Status:** Target architecture; Server foundation through schema v51, Production Runs and operational workflow-event projection, staged legacy import, Timeline, TV Dashboard, E-Ink API/simulator, Single Edit Mode, verified backup, Windows operational workspaces, and Case Operation graph validation implemented
...
The implemented foundation records migration identity in `schema_migrations` and the active version in SQLite `user_version`; ordered migrations currently reach schema version 49. Schemas v42-v44 add Haas/CNC connectivity, v45-v47 add Manufacturing Programs, Production Runs, and cycle observations, v48 adds physical-tablet status support, and v49 adds immutable operational workflow events while removing the persistent CNC mode projection. Earlier migration behavior remains unchanged.
...
`BenchAutomationService` consumes only normalized connectivity/program/counter observations; it has no Q500/Q600/SMB knowledge and no CNC workflow-mode field. Schema v49 introduces immutable `production_run_workflow_events`; Server receipt time is authoritative and any Machine timestamp is retained separately. Schema v50 adds immutable Offset Loader releases, the mutable current-release pointer, immutable sequence anomalies, and encrypted per-Machine protected-verification configuration. Schema v51 enforces one generic first-executable-block verification hook on every new approved NC upload and binds its unique six-digit NC identity to the exact immutable G-code release; it does not rewrite files or backfill historical releases. The Haas DPRNT subscriber forwards strict slash-delimited `MEIMAD/V/1` frames as raw telemetry; the alphabetic short event codes and `/` separators stay within Haas's documented DPRNT literal-text character set. Only a valid enabled/current `OLC` (`OFFSET_LOADER_COMPLETED`) frame whose release hook matches the Machine's configured invocation is currently promoted to the shared workflow stream. Missing or out-of-order sequence values are retained as anomalies and never repaired with synthetic workflow events. The tablet status projection reads the latest applicable event. Protected temporary variables remain permitted only for the later setup-verification handshake after real-controller commissioning; they do not persist workflow state. `/api/v1/machines/live` accepts an explicit Machine-ID subscription and sends normalized updates. Screens first fetch current HTTP state, then apply live notifications. Browser WebSocket reconnect never requests a CNC reconnect.
