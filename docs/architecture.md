...
The repository contains an implemented .NET 10 Server host and server-owned SQLite schema version 31. The Server owns a localhost-only Kitaron connector whose SQL client always uses `ApplicationIntent=ReadOnly`; connection testing runs `SELECT TOP (0)`, while synchronization reads the mapped planning view plus canonical `TSubOrder` order state and direct `TTreeNodes` BOM edges. Credentials remain encrypted and absent from API responses. A Ready mapping drives periodic/manual synchronization that atomically applies Cases, Orders, reusable Case Operations, and reachable Case Components. Stable source links distinguish connector-owned targets from matched manual records. `StopProduction = 1` maps to cancelled, missing connector-owned component edges deactivate, and component Cases produce derived-demand previews only. No connector path creates production planning state.
...
The implemented foundation records migration identity in `schema_migrations` and the active version in SQLite `user_version`; ordered migrations currently reach schema version 31. Schema v28 adds encrypted singleton Kitaron connection/test state, schema v29 adds the reviewed mapping and detected columns, schema v30 adds one-way sync status plus ownership-aware external links, and schema v31 adds Case Components plus Kitaron BOM sync links/counters. Earlier migration behavior remains unchanged.
