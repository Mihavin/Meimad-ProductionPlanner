...
The Timeline projection also carries additive `displayTimeZoneId`, `dayStartsAtLocal`, and `dayEndsAtLocal` display metadata. The factory-local WPF view uses them for a two-row hour ruler with header-only DAY/DARK bands. They describe the configured Timeline day window, not astronomical daylight, and are intentionally outside calculation, calendars, intervals, block identity, and Machine-row layout; older clients can ignore them. The same WPF layer locally overlays one red labelled `NOW` line when its client-clock UTC instant lies within the projection horizon. The label is converted with the configured factory timezone and a 30-second UI timer updates only that overlay. It has no Server/API/database state, scheduling effect, or assignment/interval identity; the embedded and separate Timeline surfaces each own their display marker.
