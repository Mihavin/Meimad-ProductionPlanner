...
`BenchAutomationService` consumes only normalized connectivity/program/counter observations; it has no Q500/Q600/SMB knowledge and no CNC workflow-mode field. Part counters are monitoring diagnostics and cannot mutate official output. Schema v49 introduces immutable `production_run_workflow_events`; Server receipt time is authoritative and any Machine timestamp is retained separately. Schema v50 adds immutable Offset Loader releases, the mutable current-release pointer, immutable sequence anomalies, and encrypted per-Machine protected-verification configuration. Schema v51 enforces one generic first-executable-block verification hook on every new approved NC upload and binds its unique six-digit NC identity to the exact immutable G-code release; it does not rewrite files or backfill historical releases. Schema v52 atomically creates one pending, expiring verification session only when a valid six-digit-nonce `OLC` also reports the exact stored six-digit NC identity; its immutable context binds Machine, Run, NC release, current Offset Loader, nonce, macro version, and response width. Missing/mismatched OLC or verification-result identity fails closed with a typed anomaly and no session transition. Verification success/failure additionally repeats and matches the session's release token and nonce, preventing an old delayed result from resolving a newer challenge. A newer Offset Loader supersedes any live session. For the enabled, assigned, current and unexpired context, the existing TabletID-identified tablet status service decrypts the Machine secret and derives the fixed-width response only in memory. It never projects nonce, secret/key, variable mapping, or algorithm internals, and omits the code for every invalid context. CNC protected-macro success/failure consumption is implemented with current-session, macro-version, Run, exact NC, release-token, and nonce checks. Quarantined macro versions 3-5 lack this result-correlation evidence and cannot satisfy the hardened Server contract; a separately generated macro-v6 no-motion candidate uses a third protected finalizer and one persistent increment-only sequence mapping, but remains bench-only pending written internal review and physical commissioning. Physical alarm/interlock behavior remains a commissioning gate. After Windows QC approval, strict DPRINT `CYCLE_START`/`CYCLE_END` observations require active-program identity and resolve the assigned active Run program; only an immediately consecutive matching END advances its atomic coupled outputs and durable cycle record. Duplicate ENDs are idempotent, while a reused identifier with a conflicting event type is rejected and recorded as an anomaly. Schema v56 records START/START interruption before retaining the new open attempt, and retains unmatched/nonconsecutive END facts with typed immutable anomalies and no quantity change. Missing or out-of-order sequence values on retained events are anomalies and are never repaired with synthetic workflow events. Protected temporary variables do not persist workflow state. `/api/v1/machines/live` accepts an explicit Machine-ID subscription and sends normalized updates. Screens first fetch current HTTP state, then apply live notifications. Browser WebSocket reconnect never requests a CNC reconnect.
...
Hardware and firmware remain a separate project boundary. The prototype now owns a bounded status adapter and first direct-rendered text production layout with fixed tool pages; it does not yet consume official package tools. Its `IN_SETUP` adapter requires the bounded Server verification projection, retains fixed-width response codes as text, and rejects missing/malformed/stale code combinations. The renderer replaces the ordinary tool region with either a prominent operator code or explicit expired/invalidated/unavailable `SETUP BLOCKED` instructions; it has no nonce, secret, protected-variable mapping, or response algorithm. It persists the last successfully displayed status revision, status token, and tablet identity in NVS, skips same-revision refreshes, and forces changed/reassigned refreshes. A same-tablet retained screen normally survives status failures, but a possibly visible `IN_SETUP` code is cleared once to a persisted unavailable fail-safe; the next valid response forces a repaint even at the same revision. Its power state machine maps `READY_FOR_SETUP`/`IN_SETUP` to a 15-second development timer-plus-button wake, `IN_QC` to a 120-second timer-plus-button wake, and `IN_SETUP_RUN`/`READY_FOR_PRODUCTION`/`IN_PRODUCTION` to button-only wake, then enters deep sleep without changing Server state. The shorter setup cadence prevents the physically observed 120-second polling interval from racing the minimum verification lifetime; work-window enforcement and measured battery impact remain production gates. `BLOCKED`/`UNKNOWN`/unavailable use a conservative timer fallback. EXT1 wake is compiled for all three active-low inputs. The button abstraction maps short D1 to Refresh, a 1.2-second D1 hold to Service/Debug, D2 to Previous Tool Page, short D4 to Next Tool Page, and a 1.2-second D4 hold to `SEND_TO_QC`; it debounces, requires release, logs actions, checks fresh Server eligibility, permits one POST per wake, performs a follow-up GET, and renders temporary outcome feedback. Service/Debug shows only bounded operational diagnostics and never displays credentials, verification codes, nonces, protected-variable mappings, or response-algorithm data. The verification layout, mapping, wake/current behavior, enclosure ergonomics, and confirmation display are not physically verified. Firmware also owns TabletID provisioning, full work-window scheduling, battery measurement, version polling, staged download, checksum verification, atomic activation, broader last-known-good package caching, remaining checklist/comment inputs, and device-local annotations. Those remaining behaviors and physical acceptance are not implied by the compiled state policy.
