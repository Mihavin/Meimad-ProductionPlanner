...
- **Status:** Target architecture; Server foundation through schema v53, Production Runs and operational workflow-event projection, staged legacy import, Timeline, TV Dashboard, E-Ink API/simulator, Single Edit Mode, verified backup, Windows operational workspaces, and Case Operation graph validation implemented
...
The implemented foundation records migration identity in `schema_migrations` and the active version in SQLite `user_version`; ordered migrations currently reach schema version 52. Schemas v42-v44 add Haas/CNC connectivity, v45-v47 add Manufacturing Programs, Production Runs, and cycle observations, v48 adds physical-tablet status support, v49 adds immutable operational workflow events while removing the persistent CNC mode projection, v50-v51 add immutable Offset Loader and NC-hook identities, and v52 adds one-time setup-verification sessions. Earlier migration behavior remains unchanged.
...
Official planning/package data flows Server-to-device only. An active Windows editor can generate a new official revision for assigned work; publication stages output, hashes every file, and commits immutable records only after a second Edit Mode/context check. The one approved device-originated exception is `SEND_TO_QC`: target behavior authenticates the device, resolves its bound Machine and active Production Run, records one Server-timestamped event, changes only the tablet workflow projection to `IN_QC`, and advances its status revision. Schema v49 implements the shared append-only persistence and event-derived read projection; the authenticated/idempotent POST route remains target design. Device credentials remain rejected outside assigned E-Ink reads and this future event route; there is no checklist/comment write-back, package update, or delete endpoint.
