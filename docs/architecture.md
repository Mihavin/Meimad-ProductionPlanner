...
The repository contains an implemented .NET 10 Server host and server-owned SQLite schema version 30. The Server owns a localhost-only Kitaron connector whose SQL client always uses `ApplicationIntent=ReadOnly`; connection testing runs `SELECT TOP (0)` and synchronization runs one selected-column `SELECT` against validated identifiers. Credentials remain encrypted and absent from API responses. A Ready optimistic mapping drives periodic/manual synchronization that aggregates source rows and atomically applies only Cases, Orders, and reusable Case Operations. Stable source links distinguish connector-owned targets from matched manual records and prohibit delete mirroring. No connector path creates production planning state. Planning mode remains assignment-owned and calculated dates remain transient.
...
The implemented foundation records migration identity in `schema_migrations` and the active version in SQLite `user_version`; ordered migrations currently reach schema version 30. Schema v28 adds encrypted singleton Kitaron connection/test state, schema v29 adds the reviewed mapping and detected columns, and schema v30 adds one-way sync status plus ownership-aware external links. Earlier migration behavior remains unchanged.
