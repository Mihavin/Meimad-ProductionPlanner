...
Production Package `MANUAL_DUMMY` uses the same package state machine, immutable source/package hashes, Machine binding, NC identity, and Offset Loader authorization as `MEASURED`. Only measured offset payload/artifact is absent. For CNC work, the Server requires enabled verification and atomically adds a verification-only `O01990` Offset Loader artifact containing no offset-write commands; it never silently omits the loader. The Windows preparation queue exposes the choice explicitly; the Server rejects it unless Machine configuration allows it.
