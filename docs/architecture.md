...
Kitaron source ownership is enforced in the Server persistence/application mutation path, not only in WPF. A durable Case source link makes imported Case master fields and its direct Order collection Kitaron-owned; linked Order rows carry canonical reference, quantity, supply date, source status, and the unit Price from `TSubOrder.PriceInCurr` in the Kitaron order currency. Normal clients can read these records but cannot update/delete the Case or create/update/delete its Orders. The synchronizer retains its separate internal atomic write path so it can replace the authoritative snapshot. Unlinked manually created Cases remain locally editable.

The connector mapping surface exposes canonical Order status and Price as connector-managed fields. They cannot be disabled or remapped: status is derived from the closed/`StopProduction` facts and Price is read only from `PriceInCurr`. Configurable planning-view mappings remain separate from these fixed canonical rules.
