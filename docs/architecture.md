...
The dependency-free browser simulator is served by the Server at `/eink-simulator/`. It accepts the distinct registration Device ID and operator Tablet ID required by the two scoped read projections. It uses the package version endpoint first, renders the structured Machine projection, conditionally polls package content with ETags, loads exact-revision manifests, verifies file SHA-256 in the browser, and polls physical tablet status on every refresh even when package content is unchanged. A valid `IN_SETUP`/`WAITING_FOR_OPERATOR` projection renders the real fixed-width response code plus bounded verification diagnostics; a status-contact failure clears a previously visible live code and shows a do-not-start state. Other last-known-good package content remains visible. Local-only development controls cover all seven fixed workflow states, explicitly labeled test code/failure/expiry, offline last-known-good, low battery, and revision change. The only simulator POST is the exact authenticated `SEND_TO_QC` body, enabled after a real `IN_SETUP_RUN` status; it cannot choose a Machine or Run. Structured JSON is the implemented v1 baseline; a future pre-rendered panel representation would require an explicit compatible contract decision. Physical SD staging/atomic activation, deep sleep, and device-local annotations remain firmware responsibilities and are not claimed by the simulator.
