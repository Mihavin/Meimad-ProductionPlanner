...
- **Local files.** Viewer preferences (default machine, G30, initial macro variables, program-memory folders, home offsets) are per user in `%LOCALAPPDATA%\Meimad Planner\nc-viewer\settings.json`; WebView2 data lives next to them. Programs are saved only where the user chooses (default `Documents\Meimad Planner\NC programs`); the viewer never writes into a Case Working Folder or a Server release. Tool-table edits in the viewer change only that window's preview.
