...
- **Status:** Target architecture; Server foundation through schema v56, Production Runs, operational workflow-event projection, post-QC DPRINT cycle completion/interruption evidence, staged legacy import, Timeline, TV Dashboard, E-Ink API/simulator, Single Edit Mode, verified backup, Windows operational workspaces including QC Queue, and Case Operation graph validation implemented
...
The implemented foundation records migration identity in `schema_migrations` and the active version in SQLite `user_version`; ordered migrations currently reach schema version 56. Schemas v42-v44 add Haas/CNC connectivity, v45-v47 add Manufacturing Programs, Production Runs, and cycle observations, v48 adds physical-tablet status support, v49 adds immutable operational workflow events while removing the persistent CNC mode projection, v50-v51 add immutable Offset Loader and NC-hook identities, v52 adds one-time setup-verification sessions, v53 adds bounded tablet network observations, v54 adds idempotent `SEND_TO_QC`, v55 enables event-audited QC reinspection, and v56 extends immutable anomaly evidence for interrupted/unmatched production cycles. Earlier migration behavior remains unchanged.
