...
The legacy import boundary is deliberately staged. The preview endpoint applies bounded ZIP/XML parsing and candidate lookup without Edit Mode or database writes. It keeps at most four parsed previews in Server memory for a configured lifetime and returns stable source-row keys, provenance, candidates, and issues to a four-stage Setup workflow: Cases; related Orders; Batches in Pool; then Machine assignment. These are focused views over one transient preview, not independent imports. A related Order can reference a selected Case source-row key, so the repository creates selected Cases before its related Orders in the same immediate transaction. Optional Batch-number templates, clear-Machine-suggestion acceptance, and Similar/All pattern expansion exist only in transient client preview state; the client submits the result as ordinary explicit per-row choices rather than a bulk-pattern command. A first commit accepts only an exact staged workbook hash plus those explicit operator decisions; an exact already-committed approval can be replayed from the durable receipt after staging is gone. Application/domain services revalidate all selected rows and normal business rules; the repository checks Edit authority again and commits canonical entities, full-route pool Batches, appended assignments, cross-type audit, structured event, and schema-v25 idempotency receipt in one immediate transaction. The source `.xlsx` remains external and read-only; neither the Windows client nor SQLite opens it after upload.
...
The client assembly has no SQLite reference and stores no planning data or database path. It does not open the preview path; the Server reads and streams supported preview image bytes. The Excel automatic draft is deliberately client-owned and non-authoritative: invoking it transforms only unresolved rows in the current preview into editable ordinary selections, preserves manual decisions, uses one full-quantity `stock` allocation for otherwise unlinked planning demand, creates an Order proposal only under one exact existing Case, accepts only an exact unique Machine plus a unique compatible Case-route Operation for assignment, and falls back to Pool/Skip on ambiguity. It never drafts a new Case/Working Folder or selects an existing Batch Operation. Per-row automatic-Skip reasons remain visible, and the client requires an explicit skip confirmation before enabling Commit because the schema-v25 receipt permits only one approved request for a workbook hash. The draft cannot Commit, create a Machine or route, approve an override, or update an existing entity. The existing commit endpoint remains the sole mutation boundary and independently validates every proposed mapping, allocation, route version, Operation, Machine, assignment, and backlog append in one transaction.
