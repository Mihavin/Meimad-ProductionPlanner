...
The mapping PUT body contains `modelMode` (`domain_aligned` or `flat_requested`), `status` (`draft` or `ready_for_implementation`), all catalog `fields`, optional overall `notes`, and optimistic `version`. Ready rejects blocked/missing/unknown active mappings. Sync is localhost-only, requires enabled successfully tested connection settings and a Ready mapping, and never accepts source rows from the caller. It reads only mapped source columns, caps a run at 200,000 rows, and returns `succeeded`, `failed`, or `blocked`. The Planner apply is one immediate SQLite transaction. Stable source links distinguish connector-created targets, which may receive mapped refreshes, from matched manual targets, which are never overwritten. Missing source rows are not deleted. The implemented target scope is Cases, Orders, and reusable Case Operations only; no Batch, assignment, backlog, Timeline, or Kitaron mutation endpoint exists.
