...
Success returns `schemaVersion`, `workbookSha256`, `commitId`, `replayed`, created/unchanged Case, Order, Batch, Batch Operation, and assignment IDs, `poolBatchOperationIds` for newly created route Operations left unassigned, plus imported assignment IDs by Machine in source order. An exact approved-payload replay returns the stored receipt with `replayed: true` even after preview expiry, eviction, or Server restart, provided the caller currently holds Edit Mode; a different approved payload for the same workbook returns `409 workbook_already_imported`. Expired/evicted/restarted staging for an uncommitted workbook returns `410 import_token_expired`. Other relevant errors are `400 workbook_required`, `413 workbook_too_large` or parser expansion limits, `415 unsupported_media_type`, `429 import_preview_busy`, `422 import_validation_failed` with issue details, and the normal `409` Edit Mode codes. Any validation or write failure rolls back entities, assignments, overrides, event records, and the receipt together.
