...
**Implemented now:** Case collection GET, POST, GET-by-ID, PATCH, nested operation GET/POST/PATCH, component/where-used CRUD reads and writes, component-demand preview, and image preview GET. GET-by-ID and collection results include derived `isActive`, based on `active`/`in_production` Order demand or a `waiting`/`in_production` Production Batch. Operation reorder remains Proposed.
