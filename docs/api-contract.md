...
Commit carries the normal `X-Meimad-Client-Id` and `X-Meimad-Edit-Generation` headers. Its JSON contains the exact preview `schemaVersion`, `importToken`, `workbookSha256`, approved `planningSheet`/optional `openOrdersSheet`, explicit column and Machine mappings, and one action for each approved source row. Open-order actions are `skip`, `create_case`, or `create_order`. Planning actions are `skip`, `assign_existing_operation`, `create_batch_to_pool`, or `create_batch_and_assign`. `create_case` may optionally create that new Case's fully specified Order in the same source-row action; a separate `create_order` action must reference a Case that already exists before the commit. Both Batch actions require an explicitly referenced existing routed Case, a Batch Number, and balanced explicit allocations whose public tokens are `order`, `stock`, or `scrapAllowance`. `create_batch_to_pool` snapshots the complete Case route and leaves every created Batch Operation unassigned; it requires no `caseOperationId` or Machine mapping and rejects row-level `machineId`/`compatibilityOverride` values. `create_batch_and_assign` additionally requires one explicit existing Case Operation and one active Machine, snapshots the complete route, then assigns only the selected route Operation while its siblings remain in the pool. Cross-type assignment requires `compatibilityOverride.confirmed: true` and a nonblank reason. Non-skip selections must identify every required entity; candidate suggestions and client-expanded patterns are never treated as approval. Wizard step, pattern scope, and Batch-number-template state are UI-only and are absent from this API; applying a pattern produces the same explicit row objects as manual entry.
