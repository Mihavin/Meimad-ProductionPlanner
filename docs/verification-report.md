...
- **Verdict:** The current combined Release evidence is 546 tests (360 Server and 186 Windows Client), with no failures or skips. The active Windows Excel workflow now uses the authoritative Case/Order-only mapping and submits no Batch, Operation, Machine, assignment, backlog, Timeline, or planning data. Prior compatibility and planning coverage remains green. Windows operator sign-off on a backup/rehearsed production commit and the remaining physical/operational checks remains pending.
...
|---|---:|---:|---:|
| `Meimad.Planner.Server.Tests` | 360 | 0 | 0 |
| `Meimad.Planner.Client.Windows.Tests` | 186 | 0 | 0 |
| **Total** | **546** | **0** | **0** |
