...
- **Current change review date:** 2026-08-15
- **Repository:** `Meimad-ProductionPlanner`
- **Baseline runtime tested:** .NET SDK 10.0.303, Release
- **Verdict:** The current combined Release evidence is 533 tests (352 Server and 181 Windows Client), with no failures or skips. It includes OpenCascade packaged-native STEP read plus B-rep tessellation, depth-buffered Shaded/Visible-edges/Wireframe rendering, solid center-of-gravity/Fit behavior, measurement/snapshot regression, bounded read-only legacy `.xlsx` preview, generic header detection, reviewed sheet/column re-preview, full-route unassigned-Pool Batch creation, and Edit-Mode-gated atomic/idempotent import on top of the prior assignment-owned planning, periodic PART RELOAD, canonical Timeline, employee editing, calendar-background, and current-time behavior. A real standard STEP cube additionally passed an end-to-end local import smoke as a 12-triangle solid. The supplied 45,497,003-byte workbook also passed a fresh read-only HTTP preview smoke with all 5 sheets, 16 Machine sections, 344 surfaced planning rows, and 990 open-order rows. Windows operator sign-off on representative production STEP models, a backup/rehearsed production commit, live SMTP-provider behavior, physical-device, production-service, network-security, and disaster-recovery acceptance remain manual/environmental gaps.
...
- a non-authoritative client automatic-draft action that prepares editable ordinary row decisions from the current preview: its explicit global policy changes only unresolved rows, proposes Orders only under one exact existing Case, creates no Case/Working Folder, gives eligible planning rows one full-quantity `stock` allocation, requires one exact Machine and one unique compatible Case-route Operation for assignment, and never selects an existing Batch Operation. Existing Orders, blockers, duplicates, incomplete values, and zero/multiple Case matches become reasoned Skip proposals; other ambiguity falls back to Pool when valid or Skip. Automatic-Skip reasons remain visible and require explicit confirmation before Commit because one successful receipt consumes that workbook hash. The draft neither calls Commit nor creates a Machine/route, confirms an override, or updates existing records. The existing Server commit validation and atomic/idempotent mutation path are unchanged.
...
|---|---:|---:|---:|
| `Meimad.Planner.Server.Tests` | 352 | 0 | 0 |
| `Meimad.Planner.Client.Windows.Tests` | 178 | 0 | 0 |
| **Total** | **530** | **0** | **0** |
...
The operator-supplied `data/Working plane.xlsx` was exercised through the freshly built Server's actual HTTP multipart preview endpoint against an isolated temporary database; no Edit Mode or commit endpoint was used. The 45,497,003-byte workbook exposed 5 sheets, 88 source-column descriptors, 16 distinct Machine sections, 344 planning rows, and 990 open-order rows; the JSON preview was 2,385,406 bytes. Of the planning rows, 340 contained both Part Number and quantity. Four Part-only rows remained visible with blocking `quantity_required` issues so the operator must map or Skip them; they were not silently discarded. Validation returned 5 blockers and 61 warnings, including one blocking invalid quantity, and retained cached external-formula provenance without evaluating or following external links. The original file remained unchanged; its observed SHA-256 was `6228050C77BAEB341A6CC1EAB0B98E50EF48E371E5593D1A71B5D06D90C9FCF0` and its observed UTC modified time remained `2026-08-15T06:36:55.4197101Z`.
...
This smoke establishes parser/layout compatibility for the supplied copy only. It does not approve the candidate mappings or mutate the current planning database. Before the first real commit, create and verify a Server backup, rehearse on a database copy, review all 344 row actions and every warning/Skip, reconcile resulting counts/backlogs, and obtain operator sign-off.
