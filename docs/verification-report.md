...
- **Verdict:** The current combined Release evidence is 564 tests (371 Server and 193 Windows Client), with no failures or skips. The added Server regression verifies that an installed Program Files content root redirects relative mutable storage to `%ProgramData%\MeimadPlanner\Server`; the added STEP regression verifies that Visible Edges suppresses a rear contour hidden behind a front face while Wireframe retains every mesh edge. Kitaron schema-v28/v29 coverage proves encrypted localhost-only settings, metadata detection, optimistic reviewed mappings, and stale-version validation. Schema-v30 coverage proves selected-column read-only SQL, atomic/idempotent Case/Order/Case-Operation application, persisted source ownership links and sync status, safe repeated runs, and zero Production Batch or Machine Assignment creation. Case Pool, Batch edit/delete, Excel Case/Order import, planning, Timeline, and client compatibility coverage remains green. Physical production synchronization still requires a verified backup and operator review of the first-run counts.
...
- schema v28 encrypted Server-local Kitaron connection/test configuration, schema v29 reviewed mapping storage, and schema v30 one-way sync state/source links. The four-step localhost UI can test metadata, save a Ready mapping, run synchronization, and display counts. Kitaron remains read-only; only Cases, Orders, and reusable Case Operations are applied. Missing source rows are retained, matched manual records are not overwritten, and no Batch/assignment/backlog/Timeline state is created.
