...
The task-list stabilization adds focused coverage for no-route Batch rejection, migration v26, STEP bounding/custom-reference calculations, stable one-time camera Fit, shared solid/edge projection, shaded/hidden-overlay defaults, and row-based TV projection/rendering. Final aggregate results come from the Release suites below.

- **Baseline verification date:** 2026-08-11
- **Current change review date:** 2026-08-18
- **Repository:** `Meimad-ProductionPlanner`
- **Baseline runtime tested:** .NET SDK 10.0.303, Release
- **Verdict:** The current combined Debug evidence is 572 tests (379 Server and 193 Windows Client), with no failures or skips when the two suites are run separately. Kitaron schema-v28/v29 coverage proves encrypted localhost-only settings, metadata detection, optimistic reviewed mappings, and stale-version validation. Schema-v30/v31/v32 coverage proves read-only canonical order/BOM SQL, `StopProduction = 1` cancellation, unified parent/child Case roles, explicit derived-Order Batch allocations, source ownership links, safe component deactivation, stale-link repair, full BOM-root import, and duplicate-edge consolidation. Component API coverage proves both roles remain searchable, parents reject direct Operations, children expose read-only multiplied demand, and child Batches consume it without duplicate child Orders. Physical production synchronization still requires a verified backup and operator review of first-run counts.
...
- TV is read-only; E-Ink assigned reads and the approved-but-unimplemented `SEND_TO_QC` command cannot mutate planning data;
...
| E-Ink scoped behavior | **Implemented read-side pass / compiled layout, revision, state, input, and power policies / Server SEND_TO_QC and device gaps** | Device-scoped token validation, revocation/rotation, version ETag, Machine screen, revision-qualified manifest/files, checksum enforcement, time config, simulator, and current cross-surface 403 behavior pass. Schema-v19 tests cover Timeline-derived setup worker/time, packaged worker photo, job/expected Machine tools, and local-only checklist policy. The ESP32 production and focused contract-test images compile with the bounded status parser, text-only Machine/part/Operation/status layout, three-row tool pagination model, no-image rendering, revision decisions, all seven status-to-wake policies, page boundaries, wake-button mapping, single-attempt guard, `IN_SETUP_RUN`-only send eligibility, and network-required/local-only action decisions. Firmware persists completed screen revision/tablet identity and selected tool page in NVS, skips equal status updates, preserves the retained screen on failure, logs actual `update()` duration, configures timer/EXT1 sources, debounces/releases/logs button actions, follows a guarded send with status GET, and renders temporary accepted/confirmed/rejected/uncertain notices. It also retains prior sleep policy in RTC memory, logs wake reason/available UTC time/battery/pre-sleep state, skips Wi-Fi for local button wakes, and disables the radio after HTTP work and before deep sleep. The assertions, RTC retention/pull-up current, timer cadence, GPIO actions, Wi-Fi-off behavior, battery accuracy, timestamp continuity, confirmation display, and persistence/refresh sequence have not been executed on hardware. Mandatory configured-workday/shift enforcement is not implemented, so automatic polling is development-only and not production-accepted. The `SEND_TO_QC` Server endpoint, persistence, status revision, authorization exception, and idempotency/audit tests remain unimplemented, preventing end-to-end verification. Official package tools are not yet bound to the layout. There is no checklist/comment write-back or USB endpoint. Physical panel readability/clipping, page actions, SD/error/package-last-known-good behavior, device secret storage, signatures, superseded-package retention, and hardware acceptance are not verified. |
