...
- one red labelled WPF `NOW` marker when the client current UTC instant is inside the Timeline horizon, using the configured factory timezone for display and a marker-only 30-second refresh with no API, recalculation, assignment identity, or scheduling effect.
