...
- **Verdict:** The schema-v25 combined Release run passed 496 tests (333 Server and 163 Windows Client), with no failures or skips. It adds a bounded read-only legacy `.xlsx` preview, explicit mapping/Skip workflow, and Edit-Mode-gated atomic/idempotent import on top of the prior assignment-owned planning, periodic PART RELOAD, canonical Timeline, employee editing, calendar-background, and current-time behavior. The supplied 45,497,003-byte workbook also passed a read-only HTTP preview smoke with all 5 sheets, 16 Machine sections, and 344 surfaced planning rows. Windows operator sign-off, a backup/rehearsed production commit, live SMTP-provider behavior, physical-device, production-service, network-security, and disaster-recovery acceptance remain manual/environmental gaps.
...
- schema v25 `legacy_working_plan_imports` receipts, bounded cached-value OpenXML preview, editable sheet/column/Machine/entity mappings, source-cell provenance, explicit row actions/Skip, optimistic cross-type override review, and one Edit-Mode-gated atomic commit that appends imported assignments without rescheduling or reordering existing work.
...
|---|---:|---:|---:|
| `Meimad.Planner.Server.Tests` | 333 | 0 | 0 |
| `Meimad.Planner.Client.Windows.Tests` | 163 | 0 | 0 |
| **Total** | **496** | **0** | **0** |

Current tests add schema-v25 fresh/upgrade/startup coverage; safe OpenXML formula/date/archive handling; preview/mapping/candidate and real-domain validation; atomic creation/assignment/backlog append; compatibility override/event logging; replay, changed-payload, expired-stage, bounded-stage, all-Skip, and rollback paths; typed multipart/edit-header client contracts; and Windows preview/mapping/selection/gating/refresh behavior, including Batch/part context, Case-filtered route choices, exact Machine compatibility evidence, mandatory override reason, source-column remapping, and invalid/incomplete source data. They retain the schema-v24 assignment-mode and canonical Timeline coverage described below. Automated tests do not establish visual or operational acceptance. Manual Windows import review, a backup/rehearsed production commit, production-sized contention, live measurement-source integration, and live SMTP-relay delivery remain pending. Authenticated SMTP credential storage is not implemented. Physical TV/E-Ink, production Windows Service, LAN/TLS/authentication, backup/disaster recovery, and shop-floor display checks also remain pending.
...
The operator-supplied `data/Working plane.xlsx` was exercised through the actual HTTP multipart preview endpoint against an isolated temporary database; no commit endpoint was called. The 45,497,003-byte workbook exposed 5 sheets, 16 distinct Machine sections, and 344 planning rows. Of those rows, 340 contained both Part Number and quantity. Four Part-only rows remained visible with blocking `quantity_required` issues so the operator must map or Skip them; they were not silently discarded. The parser retained cached external-formula provenance without evaluating or following external links. The original file remained unchanged; its observed SHA-256 was `6228050C77BAEB341A6CC1EAB0B98E50EF48E371E5593D1A71B5D06D90C9FCF0` and its observed UTC modified time remained `2026-08-15T06:36:55.4197101Z`.
