...
The operator-supplied `data/Working plane.xlsx` was exercised through the freshly built Server's actual HTTP multipart preview endpoint against an isolated temporary database; no Edit Mode or commit endpoint was used. The 45,497,003-byte workbook exposed 5 sheets, 16 distinct Machine sections, 344 planning rows, and 990 open-order rows; the JSON preview was 2,378,689 bytes. Of the planning rows, 340 contained both Part Number and quantity. Four Part-only rows remained visible with blocking `quantity_required` issues so the operator must map or Skip them; they were not silently discarded. The parser reported 62 warnings and retained cached external-formula provenance without evaluating or following external links. The original file remained unchanged; its observed SHA-256 was `6228050C77BAEB341A6CC1EAB0B98E50EF48E371E5593D1A71B5D06D90C9FCF0` and its observed UTC modified time remained `2026-08-15T06:36:55.4197101Z`.
