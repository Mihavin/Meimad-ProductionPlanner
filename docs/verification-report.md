...
- **2026-08-30 commissioning update:** The bounded R2 physical attempt quarantined macro v6 after alarm 903 left the sixth M109 ASCII value in `#10500`. The reviewed R3/v7 attempt corrected retained temporary authority and stopped before M30, but failed end-to-end because the physical tablet polled after expiry and SVF was not delivered before alarm 903. V8 retained an exactly correlated SVF, cleared all four temporary variables, and failed closed at alarm 903, but its Machine package was also quarantined: for OLC 29/SVF 30 the replacement tablet displayed `720047` while the exact controller package expected `812524`, proving that the Server secret and generated Machine key were not aligned. The replacement tablet worked on external power but failed on the uncommissioned AA path. `#10504 = 30` is preserved and verification is disabled. V9 recovery scaffolding preserves the V8 dwell/cleanup behavior and requires one newly rotated secret shared by Server configuration and package generation; no V9 Machine package or physical acceptance exists yet.
...
| 25 | Implemented: editor-authorized invalidation, Offset Loader revocation/replacement, tablet reassignment and enable/disable, ordinary QC retry, and attributed audit. No bypass route exists. | Physical recovery procedure rehearsal. |
...
assignment and enable/disable retain editor-attributed audit rows.
...
published self-contained `win-x64` version `0.1.48` payloads and produced both MSI
packages.
The final client package is 107,563,245 bytes (SHA-256
`A1A6B5EB7F0FE4BB8C1656245E2E20DECA3A933FC2EB082CEB3A624FFBC6C8EC`) and the
Server package is 43,534,652 bytes (SHA-256
`79AF080C22D04C56BE066ADE4CB7FB2DA467B96C8B99CED451DC8D0C985373A8`). The final
...
- tokenless E-Ink routes resolve enabled TabletID and Server-owned targets; and
...
| E-Ink scoped behavior | **Implemented read side and scoped workflow command / compiled layout, revision, state-driven power, input, battery-metadata, and backend-free demo policies / physical and telemetry-history gaps** | Tokenless TabletID resolution, disabled-tablet rejection, version ETag, Machine screen, revision-qualified manifest/files, checksum enforcement, time config, simulator, and cross-surface authorization behavior pass. Schema-v55 tests cover the exact `SEND_TO_QC` boundary. Firmware 0.1.3 and its contract image compile with a centralized policy separating screen awake state from Wi-Fi: `READY_FOR_SETUP`/`IN_SETUP`/`IN_SETUP_RUN` stay awake with Wi-Fi off and no periodic poll; D1 provides a bounded refresh/session; `IN_QC`/`IN_PRODUCTION` deep-sleep button-only; post-QA `READY_FOR_PRODUCTION` adds a configurable 60-second timer. Runtime page navigation stays local while awake, and revision gating avoids unchanged panel refreshes. The simulator exposes the same policy as explicit bench evidence; 19 focused Server/simulator tests pass. Physical assertions, RTC/pull-up current, exact timer cadence, button wake, Wi-Fi-off current, battery-only operation, page navigation, and state transitions still require device testing. Mandatory configured-workday/shift enforcement remains incomplete, so timer wake is not production-accepted outside a bounded pilot. Battery history/percentage, official package-tool binding, checklist/comment write-back, SD/package activation, and hardware acceptance remain incomplete. |
| Windows QC Queue | **Automated pass / operational identity gap** | View Mode lists only Runs whose latest event is `SEND_TO_QC`, with Machine, multi-output part/Operation summaries, receipt time, and latest packaged setup worker when known. PASS/FAIL require matching active editor client, user, and generation; invalid state/payload/reason, stale or spoofed authority, unknown Runs, and concurrent decisions are rejected. Immutable events retain user, Server UTC, and optional reason; FAIL permits a distinct resend and PASS exposes its receipt time as `production_approved_at`. Human identity headers are still development identity rather than production authentication. |
...
[Data model](data-model.md#71-machine) now consistently records that E-Ink binding and enable/disable are implemented through the device-registration API.
