...
- **Verdict:** The current combined Debug evidence is 566 tests (373 Server and 193 Windows Client), with no failures or skips when the two suites are run separately. Kitaron schema-v28/v29 coverage proves encrypted localhost-only settings, metadata detection, optimistic reviewed mappings, and stale-version validation. Schema-v30/v31 coverage proves read-only canonical order/BOM SQL, `StopProduction = 1` cancellation, atomic/idempotent Case/Order/Case-Operation/Case-Component application, source ownership links, safe component deactivation, and zero automatic child Order, Production Batch, or Machine Assignment creation. Component API coverage proves self/cycle rejection, where-used, multi-level demand, and child-Case preservation. Windows startup coverage includes the new Components workspace. Physical production synchronization still requires a verified backup and operator review of first-run counts.
