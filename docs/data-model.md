...
Schema v25 adds `legacy_working_plan_imports`. Its `workbook_sha256` is unique; `approved_request_sha256` distinguishes an exact replay from a different approval of the same workbook. The receipt also stores the successful response JSON, committing client/user IDs, and UTC commit time. Preview tokens, parsed cells, candidate lists, and workbook bytes remain bounded in-memory staging only and disappear on expiry, eviction, or Server restart. Imported Cases, Orders, Batches, and Machine Assignments are ordinary canonical records created in the same immediate transaction as the receipt and structured import event. No Excel BLOB, calculated date, invented route, or source-file path is persisted by the importer.
