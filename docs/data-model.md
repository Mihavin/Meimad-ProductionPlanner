...
The implemented migrations through schema v52 are the current persistence/domain contract. Schemas v42-v44 add Haas/CNC connection behavior, v45-v47 add Manufacturing Programs, Production Runs, and cycle observations, v48 adds physical-tablet status support, v49 adds operational workflow events while removing the CNC mode projection, v50 adds the CNC-verification identity/configuration foundation, v51 adds immutable generic NC verification-hook identity, and v52 adds one-time setup-verification sessions. Later changes require new migrations and must never rewrite an applied migration in a deployed system.
...
Schema v52 adds `cnc_setup_verification_sessions`. Each row has immutable Machine, Production Run, approved NC release, current Offset Loader release, six-digit nonce, macro version, response width, creation/expiration, and source `OFFSET_LOADER_COMPLETED` event identity. State is `PENDING`, `SUCCEEDED`, `FAILED`, `EXPIRED`, or `SUPERSEDED`; only defined forward transitions are permitted and terminal rows cannot be deleted. At most one `PENDING`/`SUCCEEDED` session exists per Machine. Creating the Offset Loader workflow event and pending session is one transaction; a newer Offset Loader atomically supersedes the prior live session. The row stores neither the Machine secret nor a response code; an authorized tablet projection derives the response in memory from this context and the separately protected Machine configuration.
