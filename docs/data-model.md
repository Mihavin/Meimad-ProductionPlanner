...
- Schema v37 adds contextual release selection, material input, and immutable offset-readiness history. It creates no material confirmation, offset confirmation, selection, or readiness boolean during migration; existing rows therefore remain safe and unknown until explicitly confirmed.
...
The implemented migrations through schema v38 are the current persistence/domain contract. Timeline expands schema-v16 timing snapshots with schema-v38 NC estimates only for not-started work, production pins preserve schema-v35 release context, schema-v36 supplies required tool positions, and schema-v37 readiness inputs are evaluated centrally for the concrete assignment. Remaining blocking model decisions in [Implementation plan](implementation-plan.md#open-decisions) must be resolved before affected repositories and APIs are implemented; later changes require new migrations and must never rewrite an applied migration in a deployed system.
