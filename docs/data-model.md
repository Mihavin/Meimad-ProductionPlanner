...
| Catalog Tool | `catalog_tools` | Schema v80: a tool definition with a stable, never-reused internal number and code (`MT-00001`), checked tool type, holder hand, JSON dimensions and attributes, active flag, version and actor; a description, not an inventory record. |
| Catalog Tool External Id | `catalog_tool_external_ids` | Schema v80: the ids of a catalog tool in other systems (supplier, ERP, CAM, presetter); unique per tool, system and value; cascade-deleted with the tool. |
| Machine | `machines` | Required Working Calendar; unique machine number. |
| Machine Assignment | `machine_assignments` | Required Batch Operation and Machine; one assignment per Batch Operation and one backlog position per Machine. Schema v24 adds required `planning_mode` constrained to `forward`, `backward`, or `manual`. Schema v69 adds nullable `released_at`; released rows are retained history and excluded from live backlog queries. |
...
Schema v3 adds nullable Order `notes`. The original v1 physical `order_reference` column now backs logical/API `orderNumber`; the unused nullable `customer_reference` column is retained for migration compatibility and is not exposed by the implemented Order slice. Schema v66 adds nullable Order `price` plus nullable `kitaron_status` (`active`, `inactive`, or `cancelled`). Schema v67 adds `kitaron_history_only` (`0` or `1`), an internal Server-owned projection marker that distinguishes the current authoritative Kitaron demand set from a superseded Order retained only for immutable production history. For Kitaron-managed Orders, `price` is the nullable unit sales price from `TSubOrder.PriceInCurr` in the Kitaron order currency; calculated row totals and manufacturing/BOM costs are not substituted. The durable Case source link owns the complete Order collection for mutation authorization, while an Order source link positively identifies a row as current authoritative demand. Therefore an unlinked legacy Order below a Kitaron Case remains read-only and is absent from current-demand collections even if its history marker is false. Order source-link keys are scoped by Kitaron Case source key plus source-row key; valid legacy unscoped links are migrated during synchronization, while a stale link targeting another Case is ignored and reconciled without redirecting the authoritative Order. A future approved rebuild may normalize the legacy physical names, but an applied migration is never rewritten.
...
`production_batches.status = 'cancelled'` is the terminal state of the explicit Batch cancellation command. Related Batch Operations and Run/Program projections become cancelled, Run output and legacy bench produced quantities become zero, and live assignments/reservations are removed in the same transaction. The Batch and allocations remain for history, but cancelled Batch allocations do not contribute to current Order lifecycle, derived demand, Planning Board, TV due-date selection, or weekly material demand. Immutable cycle and workflow event rows are retained. A Run whose outputs cross Batch boundaries cannot be reset by this command.
...
The implemented repository uses the schema-v1 `case_operations` dependency columns as a limited MVP shape: an operation has at most one referenced prior operation, and contract tokens are mapped to lowercase storage tokens. Create and optimistic PATCH rehydrate every stored row into the domain graph and validate operation uniqueness, references, dependency meanings, locked groups, and sequential acyclicity inside the same immediate transaction as the write. PATCH may change operation number, name, required Machine type, setup/cycle values, and the one-link dependency fields, but never `caseOperationId`, `caseId`, `routePosition`, or creation time. Existing Batch Operations remain unchanged on PATCH because schema v9 stores their scalar and dependency snapshots. Creating a Case Operation, however, appends a `not_started` snapshot of it (next route position, copied timing and one-link dependency fields) to every Batch of the Case whose status is not `complete` or `cancelled`, bumps each affected Batch version, and records a `production_batch_route_appended` structured event; an operation number still used by an open Batch snapshot is rejected with `batch_operation_number_in_use`. This does **not** implement arbitrary fan-in/out dependency persistence or route reordering.
...
| `toolDiameterOffsetKind` | Implemented schema-v79 token `RADIUS` (default) or `DIAMETER`: whether the control keeps cutter (D) offsets as radius or diameter values; selects what a Production Package writes from the measured diameter. |
...
Schema v78 adds nullable `machines.nc_viewer_machine`: the id of the NC engine machine definition (vendored `nc-engine\machines` or Meimad `nc-engine\meimad\machines`, for example `mazak-variaxis-i-500`) that the Windows NC viewer and the Server NC analysis interpret the Machine's programs with. Null means the engine detects the machine from the program and the Machine's `nc_dialect`. The column checks only the engine identifier grammar (lower-case letters, digits, hyphens, at most 100 characters); `MachineService` rejects an id that is not installed with the Server's NC engine (`unknown_nc_viewer_machine`).

Schema v79 adds `machines.tool_diameter_offset_kind` (`TEXT NOT NULL DEFAULT 'RADIUS'`, checked against `RADIUS` and `DIAMETER`) and the Tool Room's tool preparation: `tool_preparations` (one immutable version per save: stable ID, Batch Operation, Machine, Tool Table release, version number unique per Operation/Machine, saving client/user, Server time, optional comment, SHA-256 content hash), `tool_preparation_tools` (one row per released tool identifier: optional offset number 1–9999, measured length and diameter in mm with four decimals, shape type from the shared tool type list (the twelve v79 codes; schema v80 enlarges the list and adds the holder hand and the catalog link), a JSON object of named shape dimensions, notes) and `tool_preparation_components` (ordered components per tool: type from `HOLDER`, `EXTENSION`, `COLLET`, `ARBOR`, `SHANK`, `CUTTER`, `INSERT`, `OTHER`, name, catalog number, length, diameter, notes). Update and delete triggers make all three tables append-only. `production_packages.tool_preparation_id` (nullable restrictive foreign key) records the version a `MEASURED` package embedded; the current-package predicate additionally requires that it equals the latest version for the Operation and Machine. The same migration rebuilds `production_package_artifacts` so `artifact_type` also accepts `TOOL_OFFSETS` and `TOOL_OFFSET_PROGRAM` (rows copied, immutability triggers recreated). Measurements are Tool Room facts for package generation only; they are not warehouse or tool inventory. Readiness stays a projection: the readiness context reader joins the latest version per Operation and Machine with the required active rows of its Tool Table release, and the evaluator reports the Tool Offsets component `READY` when none of them lacks a measured length, diameter or offset number (or `MISSING` with the count that does); no `tool_offset_readiness_records` row is written by a save.

Schema v80 adds the tool catalog and enlarges the tool type list. `catalog_tools` holds one row per tool definition: stable ID, `internal_number` (unique, assigned as the next number and never reused) with its `internal_code` (`MT-` plus the five-digit number), name, `tool_type` checked against the shared list (milling: flat, ball, bull-nose, chamfer, face, slot/disc, T-slot, thread, dovetail, lollipop and engraving cutters; hole making: drill, spot drill, center drill, tap, reamer, boring head, countersink, counterbore; ISO turning: external and internal turning, external, internal and face grooving, parting, external and internal threading; probe; other; codes `END_MILL`, `BALL_END_MILL`, `BULL_NOSE_END_MILL`, `CHAMFER_MILL`, `FACE_MILL`, `SLOT_MILL`, `T_SLOT_MILL`, `THREAD_MILL`, `DOVETAIL_MILL`, `LOLLIPOP_MILL`, `ENGRAVER`, `DRILL`, `SPOT_DRILL`, `CENTER_DRILL`, `TAP`, `REAMER`, `BORING_HEAD`, `COUNTERSINK`, `COUNTERBORE`, `TURNING_TOOL`, `BORING_BAR`, `EXTERNAL_GROOVING`, `INTERNAL_GROOVING`, `FACE_GROOVING`, `PARTING`, `EXTERNAL_THREADING`, `INTERNAL_THREADING`, `PROBE`, `OTHER`), nullable `hand` (`RIGHT`, `LEFT`, `NEUTRAL`; turning tools only), description, `shape_json` (named dimensions: the v79 keys plus `cuttingWidth`, `maxDepth`, `minBoreDiameter`, `shankWidth`, `shankHeight`, `leadAngle`, `insertEdgeLength`, `pitch`, `fluteCount`), `attributes_json` (`insertCode`, `holderCode`, `threadProfile`, `material`, `coating`, `manufacturer`), `is_active`, optimistic `version`, timestamps and the saving user. `catalog_tool_external_ids` holds the tool's ids in other systems (`system`, `value`; unique per tool). The catalog is Tool Room master data: it describes tools and records no quantities, locations or stock movements (tool inventory stays out of scope). The same migration rebuilds `tool_preparation_tools` (with `tool_preparation_components`, whose foreign key follows the rebuilt table; foreign-key enforcement is off around the migration and `PRAGMA foreign_key_check` guards the commit) so `shape_type` accepts the enlarged list and the row gains `hand` and a nullable restrictive `catalog_tool_id`: a prepared tool may say which catalog tool it is, and a referenced catalog tool cannot be deleted (it is deactivated instead). Existing preparation rows are copied unchanged.

Schema v38 adds `gcode_release_analyses` keyed by G-code release and parser version, plus append-only `gcode_machine_cycle_estimates`. Analysis retains status, feed seconds, rapid distance in millimetres, tool-change count, dwell, detected units, warning/unsupported JSON, confidence, and analysis time. Each Machine estimate retains the analysis metrics, Machine rapid/tool-change/factor inputs, calculated rapid/tool-change/raw/final seconds, warning JSON, confidence, and calculation time. `effective_batch_operation_nc_estimates` is a read-only view selecting the latest estimate for the assignment's explicit current release, or its sole current compatible release when no explicit choice is needed. It does not persist a global planning duration. A release can have one analysis per parser version (append-only; update/delete triggers forbid changes): the former line parser writes `1.0.0`, the NC engine writes `nc-engine/<upstream version>+m<adapter revision>`. The newest analysis by `analyzed_at` is the release's current one for the view and the G-code catalog; a Server that gains a new analyzer version adds the missing analyses to existing releases in the background without a schema change.
...
Schema v81 (`kitaron_route_import`) adds `kitaron_stations`, the requirement step columns above, rebuilds `kitaron_sync_links` so `source_entity` admits `operation_requirement` (key `<part>\u001fstep\u001f<ActionNumber>`, next to the `case_operation` key `<part>\u001f<ActionNumber>`), and adds `requirements_created`, `requirements_updated`, `requirements_matched` and `route_steps_skipped` to `kitaron_sync_state`. A deleted Kitaron-imported requirement is recorded in `kitaron_suppressed_operations` under its requirement key (`operation_number` holds the step number). The `source_hash` of an `operation_requirement` link is the last applied Kitaron state of the step; the synchronization writes `deactivated` there when it deactivates a step Kitaron no longer lists, so the step is applied in full when it returns. For `case_operation` links the hash also covers the operation's place in the route, which is what re-applies the route sequence when Kitaron reorders a route. The synchronization writes `not-produced` there for an operation it keeps although the route no longer produces it, and `relinked` for an operation it re-links because its predecessor was removed, so the facts and the sequence are applied in full when the route produces them again. Auxiliary pins live in `resource_schedule_work` (state `PINNED`, `requested_starts_at` when the start is pinned) with one `resource_schedule_assignments` row per pinned class (`is_pinned = 1`); provisional placements are never persisted.
...
The schema-v30 authoritative cleanup has one history-preservation exception: an absent or superseded Order referenced through a Batch Allocation to a Production Run with non-null `structure_locked_at` remains stored with that locked Batch/Run. Schema v67 makes this distinction durable with `orders.kitaron_history_only`, including locked Runs connected through `production_run_outputs`. For a Kitaron-managed Case, the Case Orders collection, Case active-state Order contribution, closest-current-Order date, and new Batch-allocation eligibility require both `kitaron_history_only=0` and an Order source link; this positive-membership query also suppresses an unlinked legacy row whose marker was never set. Direct stable-ID/history reads remain available, but the parent Case link makes the complete collection Kitaron-managed and blocks ordinary PATCH/DELETE and legacy-import creation. Current snapshot membership uses the exact resolved Order target ID so a duplicate display reference cannot make a second row current. The synchronizer's obsolete set includes all noncurrent targets for Cases present in the current snapshot plus only unlinked Orders for durable Kitaron Cases omitted from a partial snapshot; this removes global orphans without inferring that an omitted linked Order is stale. Never-started dependent graphs continue to be deleted atomically.
...
- Schema v75 rebuilds `machine_connections` in place to extend the `adapter_type` check with `FANUC_FOCAS`. The migrator turns foreign-key enforcement off around that single migration so the rebuild does not cascade into `machine_current_state`, `machine_state_history`, `machine_connection_events`, or `machine_telemetry_raw`, verifies `PRAGMA foreign_key_check` before committing, and restores enforcement afterwards. The DPRNT source (`dprnt.source`, `dprnt.filePath`, `dprnt.clearPolicy`, `dprnt.port`) and the FANUC configuration live in `configuration_json`; no column was added.
- Schema v76 adds `machines.nc_dialect` (`TEXT NOT NULL DEFAULT 'HAAS_NGC'`, checked against `HAAS_NGC`, `FANUC_MACRO_B`, `MAZAK_MATRIX_EIA`, and `OKUMA_OSP`) and replaces the v61 `cnc_verification_settings_v6_mappings_insert`/`_update` triggers with dialect-aware ones that read the Machine row: Okuma OSP requires every mapping in 1–200 (`VC` common variables), FANUC macro B and Mazak Matrix EIA in 500–999, and Haas keeps 10000–10999 plus the M109 response alias (500–549 canonicalized to 10500–10549). The same migration rebuilds `cnc_verification_settings` in place (same columns, rows copied) because the v60 column CHECK on `event_sequence_variable` pinned it to 10000–10999; the rebuilt column accepts 1–10999 and the triggers apply the per-dialect range. Collision and completeness checks are unchanged. Existing Machines default to `HAAS_NGC`, so stored Haas mappings stay valid; the Machine service refuses a dialect change while that Machine's verification is enabled.
...
Schema v69 adds nullable `machine_assignments.released_at`. Unassigning an operation or cancelling a Production Batch or Production Run never deletes an assignment row, because `production_packages.machine_assignment_id` is an immutable restrictive foreign key; the row is soft-released instead (`released_at` set, `production_run_id` cleared, backlog position moved past live rows), and every current-assignment query filters `released_at IS NULL`. Schema v70 and v71 narrow the v46 `machine_assignment_run_required_update` and `machine_assignment_selected_release_matches_operation_update` triggers so they still guard live rows but permit the release update; a released row keeps its `selected_gcode_release_id` as history.
...
| Operation Resource Requirement | `operation_resource_requirements` | Case Operation requirement by class, type/capability/Skill/capacity/duration/direction/dependency; never a concrete Employee or Workstation. Schema v81 adds `name`, `step_number` (the Kitaron `ActionNumber` or a planner's step number) and `duration_per_unit_seconds` (added per planned part on top of `estimated_duration_seconds`). |
| Kitaron Station | `kitaron_stations` (v81) | One row per Kitaron `TStation.StationID` seen by the synchronization: Kitaron-owned facts (`station_name`, `station_type`, `retired`, `route_rows`, `planned_rows`, `supplier_rows`, `suggested_role`, `first_seen_at`, `last_seen_at`) and the planner's decision (`import_role` `UNDECIDED`/`MACHINE`/`WORKSTATION`/`EXTERNAL`/`IGNORE`, `machine_type`, restrictive `workstation_type_id`/`external_resource_id`, `default_minutes_per_part`, `default_minutes_per_batch`, `capacity_required`, `notes`, `decided_at`, `decided_by`, optimistic `version`). A CHECK requires the Workstation type for `WORKSTATION` and the External Resource for `EXTERNAL`. |
...
### Schema v82 - Kitaron batch authority (`kitaron_batch_authority_reset`)

- `cases.kitaron_route_locked` (0/1): set by the synchronization for Cases whose route the Kitaron route master owns; such a Case's operation list cannot be extended or shortened by a planner.
- `kitaron_sync_links.source_entity` admits `production_batch` (key `wo:<TRootCard.NUMBER>`). The batch link hash covers the work-order quantity and allocations; the marker `closed-in-kitaron` records a started batch whose work order closed.
- `kitaron_batch_material_checks(production_batch_id PK -> production_batches ON DELETE CASCADE, state IN ('available','on_order','missing','unknown'), detail, updated_at)`: Kitaron's advisory material verdict per imported batch.
