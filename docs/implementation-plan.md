...
**Status:** Schema versions 1 through 36 implemented. Verified database backup is implemented; released G-code storage must be included in the operational backup policy.

### Scope

- Implemented ordered Server schema migrations through v36. Kitaron v28 stores encrypted connection/test configuration, v29 stores the reviewed mapping and detected columns, v30 stores one-way synchronization state plus ownership-aware source links, v31-v32 implement the unified Case Component/derived-demand model, v33 synchronizes not-started Batch timing snapshots, v34 adds explicit Machine execution/capacity/timing configuration plus managed Postprocessors and Machine compatibility mappings, v35 adds released tool tables, process revisions, immutable G-code releases, and nullable exact Batch Operation production pins, and v36 adds immutable normalized released-tool rows plus a verified required-magazine-tool count. Earlier migration contracts remain unchanged.
...
**Status:** Partially implemented. Case and allocation-safe Order create/read/update, Case Operation creation and optimistic edit, Production Batch atomic creation/read/optimistic scalar-and-allocation edit/scalar-and-dependency snapshots, derived Batch and linked-Order lifecycles, Machine create/update with explicit execution/capacity/timing configuration, reusable Machine Type CRUD/linkage, managed Postprocessor CRUD and explicit Machine compatibility, Operation process-revision/G-code release/tool-table history, recurring weekly Working Calendar CRUD with usage tags/breaks/dated exceptions and one-window overnight support, dedicated Setup Calendar selection, explicit Machine Assignment services/APIs, guarded planning-record deletion, and read-only persisted Timeline projection are implemented. The Windows client exposes Setup master data, the selected Operation's release/history matrix, released tool rows/count, and live Planning Board capacity status. Case/Order/Operation/Machine deletes remain relationship-guarded; immutable process/release history additionally blocks Operation deletion. Assignment supports compatible assign, unassign, stable moves, and execution transitions. First Start pins managed exact process/G-code/tool-table context, enforces released required-tool count against usable Machine capacity for both execution modes, and enforces explicit Postprocessor compatibility for CNC work without blocking planning. Full aggregate contextual readiness, tool identity/life, offsets, material status, estimates, route reorder, arbitrary dependency fan-in/out, and aggregate route revision remain pending.
...
Implemented G-code release behavior covers approved numbered process revisions, exact immutable physical tool-table releases, immutable parsed tool rows, distinct active-required-magazine counting, Postprocessor-specific G-code numbering, local-post versus new-process confirmation, current/stale/missing projections, managed atomic/hash-verified storage, startup orphan recovery, full historical retrieval, live required-versus-available Planning Board status, transactional first-Start capacity blocking, and exact production pins. The Windows Case Operations screen exposes only a Release action, never Draft. Capacity mismatch leaves the assignment planned and requires a manual process or Machine decision; it never rewrites the program, removes tools, splits work, or moves the operation. Concurrent publication is serialized and uniqueness-constrained. Broader readiness/estimate/tool-offset/material work remains staged according to [G-code release and readiness architecture](gcode-readiness-architecture.md).
