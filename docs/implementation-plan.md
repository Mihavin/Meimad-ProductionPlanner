...
- Implemented schema versions 1-18 for the planning, Setup, resource, holiday, assignment-override, operation-time, downtime, and structured-pause model. Schema v19 extends immutable official E-Ink revisions with planned setup worker/time, worker-photo reference, job/expected Machine tools, and local checklist seed definitions. Schema v20-v21 add weekly reporting. Schema v22 adds an append-only structured planning-event stream and filtered export API without analytics or planning mutation. Schema v23 separates authoritative operation actual timing/Machine history from recalculated Timeline forecasts. Schema v24 stores checked `forward`/`backward`/`manual` intent on each Machine Assignment while leaving calculated dates transient. Schema v25 adds a durable idempotency receipt for atomic legacy-working-plan commits while keeping uploaded workbook bytes and preview staging out of SQLite. Schema v27 preserves old receipts and permits multiple workbook-plus-approved-request receipts only for guarded Case/Order-only continuation; changed planning approvals remain one-shot conflicts. Schema v28 adds the Server-local encrypted Kitaron connection/test configuration.
- Implemented ordered migration metadata, `user_version`, startup application, and rejection of newer unsupported versions.
- Implemented the Server-local Kitaron connection-management slice: localhost-only browser UI/API, encrypted password-at-rest, optimistic settings, SQL read intent, metadata-only view testing, and optional periodic connectivity checks. Source-row synchronization remains blocked by two external/domain prerequisites: the configured login must be granted access to the named database/view, and product must decide whether `WORKORDER_NUMBER + OPER_NUMBER` maps to a work-order-specific Batch Operation or whether Kitaron route rows collapse into reusable Case Operations. The read-only catalog comparison and editable initial proposal are recorded in [kitaron-database-mapping-analysis.md](kitaron-database-mapping-analysis.md) and [kitaron-initial-mapping.yaml](kitaron-initial-mapping.yaml). No importer should guess this relationship.
...
**Status:** Partially implemented. Case and allocation-safe Order create/read/update, Case Operation creation and optimistic edit, Production Batch atomic creation/read/optimistic scalar-and-allocation edit/scalar-and-dependency snapshots, derived Batch and linked-Order lifecycles, Machine create/update, reusable Machine Type CRUD/linkage, recurring weekly Working Calendar CRUD with usage tags/breaks/dated exceptions and one-window overnight support, dedicated Setup Calendar selection, explicit Machine Assignment services/APIs, guarded planning-record deletion, and read-only persisted Timeline projection are implemented. The Windows client exposes a dedicated Setup page, dynamic required-Machine options, optimistic master-data forms, and confirmation-protected deletes. Case/Order/Operation/Machine deletes remain relationship-guarded; explicitly confirmed Batch deletion owns and removes its complete database graph, compacts affected backlogs, and never touches external physical files. Assignment supports compatible assign, unassign, stable moves, and execution transitions. Orders remain demand only and no automatic scheduling is implemented. Calendar combined-overnight/overtime/archive policy, downtime mutation, route reorder, arbitrary dependency fan-in/out, and aggregate route revision remain pending.
