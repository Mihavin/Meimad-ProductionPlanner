...
- Added immutable-history reconciliation for superseded Orders: dependent never-started Batches are still removed, while Orders/Batches referenced by structure-locked Runs are retained as historical evidence, reported as synchronization warnings, and no longer abort application of current Kitaron demand. Regression coverage reproduces the legacy aggregate `3000030662` alongside canonical row `3000030662/40261` and verifies both the locked history and canonical row survive a successful sync.
