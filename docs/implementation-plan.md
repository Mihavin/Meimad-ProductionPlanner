...
**Implementation status:** Pure domain calculation and read-only Timeline API are implemented. The persistence mapper consumes current fixed Machine backlogs, recurring weekly local Machine calendars with breaks/dated exceptions or legacy explicit UTC windows, the managed Setup Calendar selection with its timezone (plus the upgrade-only legacy JSON fallback until first selection/clear), planned/open/restored Machine downtime, immutable Batch timing/dependency snapshots, quantity, and allocated-Order Work Finish Date/Order Number priority facts; the engine emits setup/production/dependency-waiting/idle/reserved/downtime intervals plus deterministic conflicts and never mutates/reorders inputs. Schema v17 and the Setup Machine Availability tab add Edit-Mode-guarded planned maintenance authoring/editing and breakdown report/restore. An open breakdown blocks through the calculation horizon; Restore closes it, and typed reason/actor/repair details remain visible. Sequential dependencies constrain the calculated child to the latest predecessor finish and emit a visible waiting interval on a free child Machine; parallel-capable and independent dependencies do not add a timing edge. When simultaneously ready operations contend for one employee, earlier Work Finish Date wins and equal dates use the naturally smaller Order Number; waiting detail exposes the decision. Current managed-calendar authoring provides multiple non-overlapping same-day work windows, contained breaks, and dated replacement schedules. When no setup calendar source is selected, setup uses Machine availability and an attention explanation identifies the fallback without suppressing operation intervals. Timezone conversion is performed on the Server for the requested horizon. The provisional duration formula is quantity multiplied by cycle time. The pure engine accepts multiple predecessor edges for one child, but schema-v9 persisted Batch snapshots expose one predecessor source field, so multi-parent authoring requires a separately approved schema/API evolution. Overnight/archive/automatic-holiday policy, downtime recurrence/cancellation, plan revision/cache, broader Work Finish Date risk/conflict policy, capability conflict projection, and production conflict severity policy remain pending.
