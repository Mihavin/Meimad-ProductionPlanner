...
- **Status:** Server foundation, SQLite schema v17, core planning-resource and Setup slices, extended Operation timing, Machine maintenance/breakdown lifecycle, derived Batch/Order lifecycles, embedded/separate-window Timeline, compact Machine Board, read-only TV Dashboard, official job-package generation, E-Ink API/simulator, Single Edit Mode, verified backup, and Windows Case/Operation/Order/Batch/Machine/Machine-Type/Calendar/Machine-Availability/administrative workspaces implemented; package approval/retention and full conflict policy remain unimplemented
...
**Status:** Partially implemented. Case and allocation-safe Order create/read/update, Case Operation creation and optimistic edit, Production Batch atomic creation/read/scalar-and-dependency snapshots, derived Batch and linked-Order lifecycles, Machine create/update, reusable Machine Type CRUD/linkage, recurring weekly Working Calendar CRUD with usage tags/breaks/dated exceptions, dedicated Setup Calendar selection, explicit Machine Assignment services/APIs, guarded planning-record deletion, and read-only persisted Timeline projection are implemented. The Windows client exposes a dedicated Setup page, dynamic required-Machine options, optimistic master-data forms, and confirmation-protected deletes. Deletes are transactionally blocked by protected relationships and never touch external files. Assignment supports compatible assign, unassign, stable moves, and execution transitions. Orders remain demand only and no automatic scheduling is implemented. Calendar overnight/archive policy, Batch field/allocation updates, downtime mutation, route reorder, arbitrary dependency fan-in/out, and aggregate route revision remain pending.
...
Implemented Machine behavior covers master-data normalization, unique numbers, stable Working Calendar and Machine Type references, normal compatibility, explicit reasoned/audited cross-type assignment override, active/inactive enforcement, optimistic PATCH, and running-backlog-head protection. Working Calendar behavior covers editor CRUD, guarded references/usages, multiple same-day work windows, contained breaks, dated exceptions, dedicated Setup selection, and optional cached Israeli-holiday overlay. Explicit online refresh writes the local cache and preserves manual overrides; Timeline/resource reads remain offline. Machine Type behavior covers unique names, reusable capabilities, rename/reference guards, optimistic update, and guarded delete. Overnight Calendar windows/archive, device binding mutation, bulk reorder, downtime, plan revisions, and full conflict calculation remain open.
...
- Add authenticated identity/session and route/Batch update, assignment-form, Working Calendar archive/overnight/automatic-holiday policy, and downtime mutation workflows against the API.
...
Current Timeline placement preserves every persisted assigned row as either earliest-feasible calculated work or operation-linked blocked waiting. An invalid, paused, or infeasible earlier row blocks every later stored backlog row from leapfrogging. Waiting distinguishes Machine/setup/day-shift calendars, skilled setup/QA/regular resources, maintenance, breakdown, pause, and sequential predecessors.
...
**Implementation status:** Pure domain calculation and read-only Timeline API are implemented. The persistence mapper reloads current SQLite Machine assignments and fixed backlog positions on every request, then consumes recurring weekly local Machine calendars with breaks/dated exceptions or legacy explicit UTC windows, the managed Setup Calendar selection with its timezone (plus the upgrade-only legacy JSON fallback until first selection/clear), planned/open/restored Machine downtime, immutable Batch timing/dependency snapshots, quantity, and allocated-Order Work Finish Date/Order Number priority facts. It requires no planned-start/planned-end fields and derives duration from setup, QA, load/unload, and cycle time multiplied by Batch planned quantity. Different operations of one Case/Batch may be assigned to different Machines. The engine emits setup/production/dependency-waiting/idle/reserved/downtime intervals plus deterministic conflicts and never mutates/reorders inputs. The Windows client invalidates Timeline after an accepted plan mutation and now replays the read if another invalidation arrives while one is in flight. Server Information/Debug logs expose Timeline input counts and per-assignment timing inputs. Schema v17 and the Setup Machine Availability tab add Edit-Mode-guarded planned maintenance authoring/editing and breakdown report/restore. An open breakdown blocks through the calculation horizon; Restore closes it, and typed reason/actor/repair details remain visible. Sequential dependencies constrain the calculated child to the latest predecessor finish and emit a visible waiting interval on a free child Machine; parallel-capable and independent dependencies do not add a timing edge. When simultaneously ready operations contend for one employee, earlier Work Finish Date wins and equal dates use the naturally smaller Order Number; waiting detail exposes the decision. Current managed-calendar authoring provides multiple non-overlapping same-day work windows, contained breaks, and dated replacement schedules. When no setup calendar source is selected, setup uses Machine availability and an attention explanation identifies the fallback without suppressing operation intervals. Timezone conversion is performed on the Server for the requested horizon. The pure engine accepts multiple predecessor edges for one child, but schema-v9 persisted Batch snapshots expose one predecessor source field, so multi-parent authoring requires a separately approved schema/API evolution. Overnight/archive/automatic-holiday policy, downtime recurrence/cancellation, plan revision/cache, broader Work Finish Date risk/conflict policy, capability conflict projection, and production conflict severity policy remain pending.

### Scope

- Implement approved duration formula, Calendar overnight policy, downtime recurrence/cancellation, Work Finish Date risk, and recalculation triggers.
...
**Implementation status:** Implemented as a compact read-only WPF projection over `GET /api/v1/timeline`. It renders setup, production, idle, downtime, and reserved intervals, one-line Machine number/name labels, visible operation name/number markers, Server conflicts, and dependency edges for only the selected Batch. The embedded tab and reusable separate window share the same live read-only view model; closing the window affects neither the planner nor its data. The client contains no calculation or scheduling rules. Plan-revision consistency, zoom, local-time selection, richer navigation, and performance targets remain open.
