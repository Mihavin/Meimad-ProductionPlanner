...
- Implemented a five-step Setup **Excel Import Wizard** for the legacy working plan: bounded/read-only `.xlsx` Preview; explicit outcome selection; sheet/column/Machine mapping; conditional row resolution plus reviewable Similar/All patterns and optional `{part}`/`{reference}`/`{row}` Batch-number templates; and Review/commit. The Server supports explicit Order creation, full-route Batch creation into the unassigned pool, and full-route Batch creation with one explicitly selected Operation assigned to a Machine. Wizard/pattern/template state is client-only; the Server receives ordinary explicit row decisions, revalidates every expanded row atomically, reports created Batch Operation and remaining-pool IDs, creates only normal domain records, appends assignments after existing backlog rows in source order, never invents routing/timing or scheduled dates, and records a schema-v25 idempotency receipt. Preview tokens are bounded, temporary Server memory and an uncommitted first attempt requires re-upload after expiry/eviction/restart; an exact committed approval remains replayable from its receipt.
