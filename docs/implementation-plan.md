...
Milestone C remains the blocking physical-controller spike. The repository now contains a read-only MDC/DPRNT evidence-capture tool and an explicit no-motion commissioning protocol in `docs/haas-protected-verification-spike.md`. Official Haas documentation confirms O9000 edit locking, G65, macro variables, alarms, and TCP DPRNT, but does not identify a supported intrinsic active/caller-program variable. The actual Machine/HFO must therefore prove that identity source, approve protected program/variable mappings, and record DPRINT/reset/power-cycle behavior before the challenge-response algorithm is selected. Milestones D-H remain future work. No current implementation claims NC identity verification, response-code calculation, CNC execution blocking, tablet code display, or production readiness.
