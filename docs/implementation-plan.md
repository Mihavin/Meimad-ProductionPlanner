...
- Implemented a five-step Setup **Excel Import Wizard** for the legacy working plan: bounded/read-only `.xlsx` Preview; explicit outcome selection; sheet/column/Machine mapping; conditional row resolution plus non-authoritative automatic draft, reviewable Similar/All patterns, and optional `{part}`/`{reference}`/`{row}` Batch-number templates; and Review/commit. Automatic draft is client-only and reversible. Invoking it applies one global policy only to unresolved rows: preserve manual decisions; create an Order proposal only under one exact existing Case; create no Case/Working Folder; assign each eligible Batch's full row quantity to `stock`; accept an automatic Machine only for an exact unique match; and assign only when one compatible Case-route Operation is uniquely determined. Existing Orders, blockers, duplicates, incomplete rows, and zero/multiple Case matches become reasoned Skip proposals; other ambiguous planning rows fall back to Pool when valid or Skip. Automatic Skip reasons remain visible and the client requires explicit skip confirmation before Commit, since one successful schema-v25 receipt consumes the workbook hash. The draft never calls Commit, selects an existing Batch Operation, creates a Machine/route, confirms an override, or updates an existing record. The Server supports explicit Order creation, full-route Batch creation into the unassigned pool, and full-route Batch creation with one explicitly selected Operation assigned to a Machine. Wizard/draft/pattern/template state is client-only; the Server receives ordinary reviewed row decisions, revalidates every expanded row atomically, reports created Batch Operation and remaining-pool IDs, creates only normal domain records, appends assignments after existing backlog rows in source order, never invents routing/timing or scheduled dates, and records a schema-v25 idempotency receipt. Preview tokens are bounded, temporary Server memory and an uncommitted first attempt requires re-upload after expiry/eviction/restart; an exact committed approval remains replayable from its receipt.
