...
- Implemented a Setup **Excel Import** workflow for the legacy working plan: bounded/read-only `.xlsx` preview; explicit sheet, column, Machine, Case, Order, Batch Operation, route Operation, allocation, override, and Skip decisions; and one Edit-Mode-gated atomic commit. It creates only normal domain records, appends assignments after existing backlog rows in source order, never invents routing/timing or scheduled dates, and records a schema-v25 idempotency receipt. Preview tokens are bounded, temporary Server memory and an uncommitted first attempt requires re-upload after expiry/eviction/restart; an exact committed approval remains replayable from its receipt.
