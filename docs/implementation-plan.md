...
- Task 11 approves voltage-only battery metadata as a separate non-planning scope. Firmware sends it on every existing ping/status/event HTTP request without changing event JSON; add authenticated Server validation, bounded per-tablet history, retention, and read/admin policy separately before introducing a telemetry POST route.
...
- **OD-025 - Telemetry and tablet-originated events:** Partially resolved: `SEND_TO_QC` is the only tablet-originated operational command. It requires an enabled path-matched device credential, takes no target/time fields, resolves the bound Machine and unique `IN_SETUP_RUN` Production Run on the Server, atomically records one append-only event/audit entry using Server UTC, changes only the tablet workflow projection to `IN_QC`, and is idempotent per Production Run while retaining the first timestamp. It does not require Single Edit Mode and cannot mutate planning, Production Run lifecycle/program/output facts, readiness, or packages. Firmware physical-button binding, fresh-state eligibility, single-wake submission guard, follow-up refresh, and temporary confirmation rendering are compiled; physical/end-to-end verification is pending. Task 11 separately approves voltage-only device-health metadata on existing firmware HTTP requests; it has no planning/workflow effect and currently has no Server persistence. Define authenticated ingestion, history retention/read access, firmware/last-seen fields, hardware-range validation, and battery percentage calibration. Server endpoint/migration/status projection remain implementation work, as do the other tablet status derivations and the later Server-owned transition out of `IN_QC`.
