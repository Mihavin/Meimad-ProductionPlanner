...
Schema v49 adds a distinct append-only operational workflow stream for setup verification, QC, cycle/session observations, and `SEND_TO_QC`. Server receipt time is authoritative; Machine time and source sequence are evidence, not replacements for it. The persistent CNC Setup/Production mode variable is removed and cannot move a run between workflow states. Protected temporary variables are supported only inside the separately commissioned setup-verification handshake and are never the durable workflow projection; their configured Machine-specific mapping and physical fail-closed behavior remain commissioning gates.
