...
The dashboard is dependency-free and requires no separate web host or build step. Its no-scroll grid displays only each display-enabled active Machine's number, name, and status. Grid dimensions and text density adapt to the Machine count and viewport, and a small green/yellow/red dot is the only visible Server state. It has no host text, summaries, job details, configuration, forms, edit controls, Edit Mode calls, or planning mutation code.
