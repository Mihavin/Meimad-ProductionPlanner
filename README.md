...
`GET /api/v1/timeline?from=<RFC3339>&to=<RFC3339>` reloads every persisted Machine assignment and places each valid operation at its earliest feasible calculated time in unchanged stored backlog order. It applies Machine, Setup, day-shift, and employee calendars; breaks, exceptions, cached holidays, maintenance/breakdown; setup/QA/regular-worker availability and skills; pause state; and dependencies. Work may split across availability windows. Every delay is operation-linked waiting with an explanatory tooltip. Assigned work that cannot be fully placed in the requested horizon remains visible as blocked waiting with its structured conflict instead of making the Machine look merely Idle; an earlier blocked row prevents later backlog work from leapfrogging. Legacy setup JSON remains an upgrade fallback until the first managed selection or explicit clear. Employee contention uses earlier Work Finish Date, then naturally smaller Order Number, without changing backlog order. The compact Windows view splits exactly overlapping blocked intervals into small lanes so their labels/tooltips remain accessible. The same live read-only view can open in a separate window whose close action does not affect the main client or planning state.
