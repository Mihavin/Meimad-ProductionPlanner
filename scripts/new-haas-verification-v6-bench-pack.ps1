...
if (-not $AcknowledgeBenchOnlyCandidate) {
    throw @'
Generation is disabled by default. Macro v6 is an internally reviewable,
...
$macroVersion = Require-IntegerRange macroVersion $config.macroVersion 6 999999
...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    ...
    $OutputDirectory = Join-Path $repositoryRoot ".diagnostics\haas-v6-bench\$machineLabel"
}
...
$challengeTemplate = @'
%
O0{{CHALLENGE}} (MEIMAD PROTECTED CHALLENGE V6)
...
$verifyTemplate = @'
%
O0{{VERIFY}} (MEIMAD PROTECTED VERIFY INPUT V6)
(NO MOTION BENCH CANDIDATE - INTERNAL REVIEW AND PHYSICAL RETEST REQUIRED)
...
$finalizeTemplate = @'
%
O0{{FINALIZE}} (MEIMAD PROTECTED VERIFY FINALIZER V6)
...
$testOffsetTemplate = @'
%
O0{{TEST_OFFSET}} (MEIMAD V6 NO-MOTION TEST OFFSET LOADER)
...
$testNcTemplate = @'
%
O0{{TEST_NC}} (MEIMAD V6 NO-MOTION TEST NC PROGRAM)
...
$files = [ordered]@{
    ('O0{0:D4}-CHALLENGE-V6.CNC' -f $challengeProgram) = Render-Template $challengeTemplate $values
    ('O0{0:D4}-VERIFY-INPUT-V6.CNC' -f $verifyProgram) = Render-Template $verifyTemplate $values
    ('O0{0:D4}-VERIFY-FINALIZER-V6.CNC' -f $finalizeProgram) = Render-Template $finalizeTemplate $values
...
Write-Host "Generated internally reviewable v6 bench pack: $outputFullPath"
